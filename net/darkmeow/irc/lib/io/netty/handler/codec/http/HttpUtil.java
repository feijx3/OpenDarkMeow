...
public final class HttpUtil {
    ...
    public static long normalizeAndGetContentLength(List<? extends CharSequence> contentLengthFields, boolean isHttp10OrEarlier, boolean allowDuplicateContentLengths) {
        ...
        boolean bl2 = multipleContentLengths = contentLengthFields.size() > 1 || firstField.indexOf(44) >= 0;
        ...
        try {
            ...
            throw new IllegalArgumentException("Content-Length value is not a number: " + firstField, e2);
        }
    }
    ...
    private static int validateAsciiStringToken(AsciiString token) {
        ...
        for (int i2 = token.arrayOffset(); i2 < len; ++i2) {
            if (BitSet128.contains(array[i2], TOKEN_CHARS_HIGH, TOKEN_CHARS_LOW)) continue;
            return i2 - token.arrayOffset();
        }
        return -1;
    }
    ...
    private static int validateCharSequenceToken(CharSequence token) {
        ...
        for (int i2 = 0; i2 < len; ++i2) {
            byte value = (byte)token.charAt(i2);
