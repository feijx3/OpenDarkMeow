...
final class ChannelHandlerMask {
    ...
    static int mask(Class<? extends ChannelHandler> clazz) {
        Map<Class<? extends ChannelHandler>, Integer> cache2 = MASKS.get();
        Integer mask = cache2.get(clazz);
        if (mask == null) {
            mask = ChannelHandlerMask.mask0(clazz);
            cache2.put(clazz, mask);
        }
        return mask;
    }
    ...
    private static boolean isSkippable(final Class<?> handlerType, final String methodName, final Class<?> ... paramTypes) throws Exception {
        return AccessController.doPrivileged(new PrivilegedExceptionAction<Boolean>(){
            ...
            public Boolean run() throws Exception {
                Method m2;
                try {
                    m2 = handlerType.getMethod(methodName, paramTypes);
                }
                catch (NoSuchMethodException e2) {
                    if (logger.isDebugEnabled()) {
                        logger.debug("Class {} missing method {}, assume we can not skip execution", handlerType, methodName, e2);
                    }
                    return false;
                }
                return m2.isAnnotationPresent(Skip.class);
            }
        });
    }
