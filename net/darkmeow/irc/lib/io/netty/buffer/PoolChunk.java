...
import net.darkmeow.irc.lib.io.netty.buffer.IntPriorityQueue;
import net.darkmeow.irc.lib.io.netty.buffer.LongLongHashMap;
...
implements PoolChunkMetric {
    ...
    boolean allocate(PooledByteBuf<T> buf, int reqCapacity, int sizeIdx, PoolThreadCache cache2) {
            ...
            if (sizeIdx <= this.arena.sizeClass.smallMaxSizeIdx) {
                ...
                try {
                    ...
                    if (nextSub != head) {
                        ...
                        nextSub.chunk.initBufWithSubpage(buf, null, handle2, reqCapacity, cache2);
                        boolean bl2 = true;
                        return bl2;
                    }
        ...
        this.initBuf(buf, nioBuffer, handle, reqCapacity, cache2);
        return true;
    }
    ...
    void initBufWithSubpage(PooledByteBuf<T> buf, ByteBuffer nioBuffer, long handle, int reqCapacity, PoolThreadCache threadCache) {
        ...
        assert (s2.isDoNotDestroy());
        assert (reqCapacity <= s2.elemSize) : reqCapacity + "<=" + s2.elemSize;
        int offset = (runOffset << this.pageShifts) + bitmapIdx * s2.elemSize;
        buf.init(this, nioBuffer, handle, offset, reqCapacity, s2.elemSize, threadCache);
    }
