...
implements SizeClassesMetric {
    ...
    private static short[] newSizeClass(int index, int log2Group, int log2Delta, int nDelta, int pageShifts) {
        ...
        int n2 = log2Size = log2Delta + log2Ndelta == log2Group ? log2Group + 1 : log2Group;
    ...
    private static int[] newIdx2SizeTab(short[][] sizeClasses, int nSizes, int directMemoryCacheAlignment) {
        ...
        for (int i2 = 0; i2 < nSizes; ++i2) {
            short[] sizeClass = sizeClasses[i2];
            sizeIdx2sizeTab[i2] = SizeClasses.sizeOf(sizeClass, directMemoryCacheAlignment);
        }
    ...
    private static int[] newSize2idxTab(int lookupMaxSize, short[][] sizeClasses) {
        ...
        while (size <= lookupMaxSize) {
            short log2Delta = sizeClasses[i2][2];
    ...
    public int size2SizeIdx(int size) {
        ...
        int x2 = PoolThreadCache.log2((size << 1) - 1);
        int shift = x2 < 7 ? 0 : x2 - 6;
        int group = shift << 2;
        int log2Delta = x2 < 7 ? 4 : x2 - 2 - 1;
    ...
    private int pages2pageIdxCompute(int pages, boolean floor) {
        ...
        int x2 = PoolThreadCache.log2((pageSize << 1) - 1);
        int shift = x2 < 2 + this.pageShifts ? 0 : x2 - (2 + this.pageShifts);
        int group = shift << 2;
        int log2Delta = x2 < 2 + this.pageShifts + 1 ? this.pageShifts : x2 - 2 - 1;
