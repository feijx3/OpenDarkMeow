...
import net.darkmeow.irc.lib.io.netty.buffer.AdaptiveByteBufAllocator;
...
public final class ByteBufUtil {
    ...
    static byte[] threadLocalTempArray(int minLength) {
        if (minLength <= 1024 && Thread.currentThread() instanceof FastThreadLocalThread) {
    ...
    public static int indexOf(ByteBuf needle, ByteBuf haystack) {
        ...
        int n2 = haystack.readableBytes();
        int m2 = needle.readableBytes();
        if (m2 == 0) {
            return 0;
        }
        if (m2 == 1) {
        ...
        long suffixes = ByteBufUtil.maxSuf(needle, m2, aStartIndex, true);
        long prefixes = ByteBufUtil.maxSuf(needle, m2, aStartIndex, false);
        int ell = Math.max((int)(suffixes >> 32), (int)(prefixes >> 32));
        int per = Math.max((int)suffixes, (int)prefixes);
        if (ByteBufUtil.equals(needle, aStartIndex, needle, aStartIndex + per, length = Math.min(m2 - per, ell + 1))) {
            int memory = -1;
            while (j2 <= n2 - m2) {
                int i2;
                for (i2 = Math.max(ell, memory) + 1; i2 < m2 && needle.getByte(i2 + aStartIndex) == haystack.getByte(i2 + j2 + bStartIndex); ++i2) {
                }
                if (i2 > n2) {
                    return -1;
                }
                if (i2 >= m2) {
                    for (i2 = ell; i2 > memory && needle.getByte(i2 + aStartIndex) == haystack.getByte(i2 + j2 + bStartIndex); --i2) {
                    }
                    if (i2 <= memory) {
            ...
            per = Math.max(ell + 1, m2 - ell - 1) + 1;
            while (j2 <= n2 - m2) {
                int i3;
                for (i3 = ell + 1; i3 < m2 && needle.getByte(i3 + aStartIndex) == haystack.getByte(i3 + j2 + bStartIndex); ++i3) {
                }
                if (i3 > n2) {
                    return -1;
                }
                if (i3 >= m2) {
                    for (i3 = ell; i3 >= 0 && needle.getByte(i3 + aStartIndex) == haystack.getByte(i3 + j2 + bStartIndex); --i3) {
                    }
                    if (i3 < 0) {
                        return j2 + bStartIndex;
                    }
    ...
    private static long maxSuf(ByteBuf x2, int m2, int start, boolean isSuffix) {
        int p2 = 1;
        int ms = -1;
        int j2 = start;
        int k2 = 1;
        while (j2 + k2 < m2) {
            boolean suffix;
            byte a2 = x2.getByte(j2 + k2);
            byte b2 = x2.getByte(ms + k2);
            boolean bl2 = isSuffix ? a2 < b2 : (suffix = a2 > b2);
            if (suffix) {
                k2 = 1;
                p2 = (j2 += k2) - ms;
                continue;
    ...
    private static long compareUintBigEndianA(ByteBuf bufferA, ByteBuf bufferB, int aIndex, int bIndex, int uintCountIncrement) {
        ...
        while (aIndex < aEnd) {
            long b2;
            long a2 = bufferA.getUnsignedInt(aIndex);
            long comp = a2 - (b2 = ByteBufUtil.uintFromLE(bufferB.getUnsignedIntLE(bIndex)));
            if (comp != 0L) {
                return comp;
            }
            aIndex += 4;
    ...
    private static long compareUintBigEndianB(ByteBuf bufferA, ByteBuf bufferB, int aIndex, int bIndex, int uintCountIncrement) {
        ...
        while (aIndex < aEnd) {
            long b2;
            long a2 = ByteBufUtil.uintFromLE(bufferA.getUnsignedIntLE(aIndex));
            long comp = a2 - (b2 = bufferB.getUnsignedInt(bIndex));
            if (comp != 0L) {
                return comp;
            }
            aIndex += 4;
    ...
    private static int linearFirstIndexOf(AbstractByteBuf buffer, int fromIndex, int toIndex, byte value) {
        for (int i2 = fromIndex; i2 < toIndex; ++i2) {
            if (buffer._getByte(i2) != value) continue;
            return i2;
        }
        return -1;
    }
    ...
    private static int safeDirectWriteUtf8(ByteBuffer buffer, int writerIndex, CharSequence seq, int start, int end) {
        ...
        for (int i2 = start; i2 < end; ++i2) {
            ...
            if (c2 < '\u0080') {
                buffer.put(writerIndex++, (byte)c2);
                continue;
            }
            if (c2 < '\u0800') {
                buffer.put(writerIndex++, (byte)(0xC0 | c2 >> 6));
                buffer.put(writerIndex++, (byte)(0x80 | c2 & 0x3F));
                continue;
            }
            if (StringUtil.isSurrogate(c2)) {
                ...
                char c22 = seq.charAt(i2);
                if (!Character.isLowSurrogate(c22)) {
                    buffer.put(writerIndex++, (byte)63);
                    buffer.put(writerIndex++, (byte)(Character.isHighSurrogate(c22) ? 63 : (byte)c22));
                    continue;
                }
            ...
            buffer.put(writerIndex++, (byte)(0xE0 | c2 >> 12));
            buffer.put(writerIndex++, (byte)(0x80 | c2 >> 6 & 0x3F));
            buffer.put(writerIndex++, (byte)(0x80 | c2 & 0x3F));
        }
    ...
    private static int safeArrayWriteUtf8(byte[] buffer, int writerIndex, CharSequence seq, int start, int end) {
        ...
        for (int i2 = start; i2 < end; ++i2) {
            char c2 = seq.charAt(i2);
            if (c2 < '\u0080') {
                buffer[writerIndex++] = (byte)c2;
                continue;
            }
            if (c2 < '\u0800') {
                buffer[writerIndex++] = (byte)(0xC0 | c2 >> 6);
                buffer[writerIndex++] = (byte)(0x80 | c2 & 0x3F);
                continue;
            }
            if (StringUtil.isSurrogate(c2)) {
                if (!Character.isHighSurrogate(c2)) {
                ...
                if (!Character.isLowSurrogate(c22)) {
                    buffer[writerIndex++] = 63;
                    buffer[writerIndex++] = (byte)(Character.isHighSurrogate(c22) ? 63 : (int)c22);
                    continue;
                }
                int codePoint = Character.toCodePoint(c2, c22);
            ...
            buffer[writerIndex++] = (byte)(0xE0 | c2 >> 12);
            buffer[writerIndex++] = (byte)(0x80 | c2 >> 6 & 0x3F);
            buffer[writerIndex++] = (byte)(0x80 | c2 & 0x3F);
        }
        return writerIndex - oldWriterIndex;
    }
    ...
    private static int unsafeWriteUtf8(byte[] buffer, long memoryOffset, int writerIndex, CharSequence seq, int start, int end) {
        ...
        for (int i2 = start; i2 < end; ++i2) {
            char c2 = seq.charAt(i2);
            if (c2 < '\u0080') {
                PlatformDependent.putByte((Object)buffer, writerOffset++, (byte)c2);
                continue;
            }
            if (c2 < '\u0800') {
                PlatformDependent.putByte((Object)buffer, writerOffset++, (byte)(0xC0 | c2 >> 6));
                PlatformDependent.putByte((Object)buffer, writerOffset++, (byte)(0x80 | c2 & 0x3F));
                continue;
            }
            if (StringUtil.isSurrogate(c2)) {
                if (!Character.isHighSurrogate(c2)) {
                ...
                if (!Character.isLowSurrogate(c22)) {
                    PlatformDependent.putByte((Object)buffer, writerOffset++, (byte)63);
                    PlatformDependent.putByte((Object)buffer, writerOffset++, (byte)(Character.isHighSurrogate(c22) ? 63 : (int)c22));
                    continue;
                }
                int codePoint = Character.toCodePoint(c2, c22);
            ...
            PlatformDependent.putByte((Object)buffer, writerOffset++, (byte)(0xE0 | c2 >> 12));
            PlatformDependent.putByte((Object)buffer, writerOffset++, (byte)(0x80 | c2 >> 6 & 0x3F));
            PlatformDependent.putByte((Object)buffer, writerOffset++, (byte)(0x80 | c2 & 0x3F));
        }
    ...
    private static int utf8ByteCount(CharSequence seq, int start, int end) {
        ...
        for (i2 = start; i2 < end && seq.charAt(i2) < '\u0080'; ++i2) {
        }
        return i2 < end ? i2 - start + ByteBufUtil.utf8BytesNonAscii(seq, i2, end) : i2 - start;
    }
    ...
    private static int utf8BytesNonAscii(CharSequence seq, int start, int end) {
        ...
        for (int i2 = start; i2 < end; ++i2) {
            ...
            if (StringUtil.isSurrogate(c2)) {
                ...
                if (!Character.isLowSurrogate(seq.charAt(i2))) {
                    encodedLength += 2;
                    continue;
                }
                encodedLength += 4;
    ...
    static {
        ...
        String allocType = SystemPropertyUtil.get("net.darkmeow.irc.lib.io.netty.allocator.type", "adaptive");
        ...
        THREAD_LOCAL_BUFFER_SIZE = SystemPropertyUtil.getInt("net.darkmeow.irc.lib.io.netty.threadLocalDirectBufferSize", 0);
        logger.debug("-Dio.netty.threadLocalDirectBufferSize: {}", (Object)THREAD_LOCAL_BUFFER_SIZE);
        MAX_CHAR_BUFFER_SIZE = SystemPropertyUtil.getInt("net.darkmeow.irc.lib.io.netty.maxThreadLocalCharBufferSize", 16384);
        ...
        private static void appendPrettyHexDump(StringBuilder dump, ByteBuf buf, int offset, int length) {
            ...
            for (int row = 0; row < fullRows; ++row) {
                ...
                for (j2 = rowStartIndex; j2 < rowEndIndex; ++j2) {
                    dump.append(BYTE2HEX[buf.getUnsignedByte(j2)]);
                }
                dump.append(" |");
                for (j2 = rowStartIndex; j2 < rowEndIndex; ++j2) {
