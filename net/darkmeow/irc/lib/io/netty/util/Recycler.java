...
public abstract class Recycler<T> {
    ...
    public final T get() {
        Object obj;
        if (this.maxCapacityPerThread == 0 || PlatformDependent.isVirtualThread(Thread.currentThread())) {
    ...
    static {
        int maxCapacityPerThread = SystemPropertyUtil.getInt("net.darkmeow.irc.lib.io.netty.recycler.maxCapacityPerThread", SystemPropertyUtil.getInt("net.darkmeow.irc.lib.io.netty.recycler.maxCapacity", 4096));
        if (maxCapacityPerThread < 0) {
            maxCapacityPerThread = 4096;
        }
        DEFAULT_MAX_CAPACITY_PER_THREAD = maxCapacityPerThread;
        DEFAULT_QUEUE_CHUNK_SIZE_PER_THREAD = SystemPropertyUtil.getInt("net.darkmeow.irc.lib.io.netty.recycler.chunkSize", 32);
        RATIO = Math.max(0, SystemPropertyUtil.getInt("net.darkmeow.irc.lib.io.netty.recycler.ratio", 8));
        BLOCKING_POOL = SystemPropertyUtil.getBoolean("net.darkmeow.irc.lib.io.netty.recycler.blocking", false);
        BATCH_FAST_TL_ONLY = SystemPropertyUtil.getBoolean("net.darkmeow.irc.lib.io.netty.recycler.batchFastThreadLocalOnly", true);
    ...
    implements MessagePassingQueue<T> {
        ...
        @Override
        public synchronized boolean offer(T e2) {
            if (this.deque.size() == this.maxCapacity) {
                return false;
            }
            return this.deque.offer(e2);
        }
        ...
        @Override
        public boolean relaxedOffer(T e2) {
        ...
        @Override
        public int drain(MessagePassingQueue.Consumer<T> c2, int limit) {
            T obj;
            int i2;
            for (i2 = 0; i2 < limit && (obj = this.poll()) != null; ++i2) {
                c2.accept(obj);
            }
            return i2;
        }
        ...
        @Override
        public int fill(MessagePassingQueue.Supplier<T> s2, int limit) {
            throw new UnsupportedOperationException();
        }

        @Override
        public int drain(MessagePassingQueue.Consumer<T> c2) {
            throw new UnsupportedOperationException();
        }

        @Override
        public int fill(MessagePassingQueue.Supplier<T> s2) {
            throw new UnsupportedOperationException();
        }

        @Override
        public void drain(MessagePassingQueue.Consumer<T> c2, MessagePassingQueue.WaitStrategy wait, MessagePassingQueue.ExitCondition exit) {
            throw new UnsupportedOperationException();
        }

        @Override
        public void fill(MessagePassingQueue.Supplier<T> s2, MessagePassingQueue.WaitStrategy wait, MessagePassingQueue.ExitCondition exit) {
            throw new UnsupportedOperationException();
        }
    }
    ...
    implements MessagePassingQueue.Consumer<DefaultHandle<T>> {
        ...
        DefaultHandle<T> claim() {
            ...
            if (null != (handle = this.batch.pollLast())) {
                handle.toClaimed();
            }
            return handle;
        }
        ...
        @Override
        public void accept(DefaultHandle<T> e2) {
            this.batch.addLast(e2);
        }
    }
