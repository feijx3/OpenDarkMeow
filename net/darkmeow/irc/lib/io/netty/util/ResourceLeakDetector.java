...
import net.darkmeow.irc.lib.io.netty.util.ResourceLeak;
...
public class ResourceLeakDetector<T> {
    private static final String PROP_LEVEL_OLD = "net.darkmeow.irc.lib.io.netty.leakDetectionLevel";
    private static final String PROP_LEVEL = "net.darkmeow.irc.lib.io.netty.leakDetection.level";
    private static final Level DEFAULT_LEVEL;
    private static final String PROP_TARGET_RECORDS = "net.darkmeow.irc.lib.io.netty.leakDetection.targetRecords";
    private static final int DEFAULT_TARGET_RECORDS = 4;
    private static final String PROP_SAMPLING_INTERVAL = "net.darkmeow.irc.lib.io.netty.leakDetection.samplingInterval";
    ...
    public static void addExclusions(Class clz, String ... methodNames) {
        ...
        for (int i2 = 0; !(i2 >= n2 || nameSet.remove((method = methodArray[i2]).getName()) && nameSet.isEmpty()); ++i2) {
        }
        if (!nameSet.isEmpty()) {
        ...
        do {
            ...
            for (int i3 = 0; i3 < methodNames.length; ++i3) {
                newMethods[oldMethods.length + i3 * 2] = clz.getName();
                newMethods[oldMethods.length + i3 * 2 + 1] = methodNames[i3];
            }
    ...
    static {
        ...
        if (SystemPropertyUtil.get("net.darkmeow.irc.lib.io.netty.noResourceLeakDetection") != null) {
            disabled = SystemPropertyUtil.getBoolean("net.darkmeow.irc.lib.io.netty.noResourceLeakDetection", false);
    ...
    extends Throwable {
        ...
        public String toString() {
            ...
            block0: for (int i2 = 3; i2 < array.length; ++i2) {
                ...
                for (int k2 = 0; k2 < exclusions.length; k2 += 2) {
                    if (exclusions[k2].equals(element.getClassName()) && exclusions[k2 + 1].equals(element.getMethodName())) continue block0;
                }
                buf.append('\t');
            ...
            while (oldHead != TraceRecord.BOTTOM) {
                ...
                if (seen.add(s2)) {
                    if (oldHead.next == TraceRecord.BOTTOM) {
                        ...
                        buf.append('#').append(i2++).append(':').append(StringUtil.NEWLINE).append(s2);
                    }
                } else {
                    ++duped;
                }
