...
extends ByteBuf {
    ...
    public long[] readLongArray(@Nullable long[] array, int maxLength) {
        ...
        if (array == null || array.length != i2) {
            if (i2 > maxLength) {
                throw new DecoderException("LongArray with size " + i2 + " is bigger than allowed " + maxLength);
            }
            array = new long[i2];
        }
        for (int j2 = 0; j2 < array.length; ++j2) {
            array[j2] = this.readLong();
        }
        return array;
    }
    ...
    @NotNull
    public DataUser readUser() {
        return new DataUser(this.readString(100), this.readEnumValue(EnumUserPremium.class), this.readUserState());
    }

    @NotNull
    public FriendBuffer writeUser(DataUser data) {
        this.writeString(data.getName());
        this.writeEnumValue(data.getPremium());
        this.writeUserState(data.getState());
        return this;
    }

    @NotNull
    public DataUserState readUserState() {
        return new DataUserState(this.readPlayInGameProfile(), this.readString(100), this.readInt(), this.readBoolean());
    }

    @NotNull
    public FriendBuffer writeUserState(DataUserState data) {
        this.writePlayInGameProfile(data.getProfile());
        this.writeString(data.getCurrentServer());
        this.writeInt(data.getClientFps());
        this.writeBoolean(data.isFriend());
        return this;
    }

    @NotNull
    public DataPlayInGameProfile readPlayInGameProfile() {
        return new DataPlayInGameProfile(this.readString(100), this.readUniqueId());
    }

    @NotNull
    public FriendBuffer writePlayInGameProfile(DataPlayInGameProfile data) {
        this.writeString(data.getName());
    ...
    @NotNull
    public DataClientBrand readClientBrand() {
        return new DataClientBrand(this.readString(100), this.readString(100), this.readInt());
    }

    @NotNull
    public FriendBuffer writeClientBrand(DataClientBrand data) {
        this.writeString(data.getName());
