...
extends ReflectionFactory {
    ...
    @Override
    public KProperty0 property0(PropertyReference0 p2) {
        return new KProperty0Impl(ReflectionFactoryImpl.getOwner(p2), p2.getName(), p2.getSignature(), p2.getBoundReceiver());
    }

    @Override
    public KMutableProperty0 mutableProperty0(MutablePropertyReference0 p2) {
        return new KMutableProperty0Impl(ReflectionFactoryImpl.getOwner(p2), p2.getName(), p2.getSignature(), p2.getBoundReceiver());
    }

    @Override
    public KProperty1 property1(PropertyReference1 p2) {
        return new KProperty1Impl(ReflectionFactoryImpl.getOwner(p2), p2.getName(), p2.getSignature(), p2.getBoundReceiver());
    }

    @Override
    public KMutableProperty1 mutableProperty1(MutablePropertyReference1 p2) {
        return new KMutableProperty1Impl(ReflectionFactoryImpl.getOwner(p2), p2.getName(), p2.getSignature(), p2.getBoundReceiver());
    }

    @Override
    public KProperty2 property2(PropertyReference2 p2) {
        return new KProperty2Impl(ReflectionFactoryImpl.getOwner(p2), p2.getName(), p2.getSignature());
    }

    @Override
    public KMutableProperty2 mutableProperty2(MutablePropertyReference2 p2) {
        return new KMutableProperty2Impl(ReflectionFactoryImpl.getOwner(p2), p2.getName(), p2.getSignature());
    }
    ...
    public KType typeOf(KClassifier klass, List<KTypeProjection> arguments, boolean isMarkedNullable) {
        if (klass instanceof ClassBasedDeclarationContainer) {
            return CachesKt.getOrCreateKType(((ClassBasedDeclarationContainer)((Object)klass)).getJClass(), arguments, isMarkedNullable);
        }
        return KClassifiers.createType(klass, arguments, isMarkedNullable, Collections.emptyList());
    }

    @Override
    public KTypeParameter typeParameter(Object container, String name, KVariance variance, boolean isReified2) {
