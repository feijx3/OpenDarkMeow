...
implements Comparator<DeclarationDescriptor> {
    ...
    public int compare(DeclarationDescriptor o1, DeclarationDescriptor o2) {
        ...
        if (o1 instanceof TypeAliasDescriptor && o2 instanceof TypeAliasDescriptor) {
            ...
            for (int i2 = 0; i2 < Math.min(c1ValueParameters.size(), c2ValueParameters.size()); ++i2) {
                String p2;
                String p1 = RENDERER.renderType(c1ValueParameters.get(i2).getType());
                int parametersCompareTo = p1.compareTo(p2 = RENDERER.renderType(c2ValueParameters.get(i2).getType()));
            ...
            for (int i3 = 0; i3 < Math.min(c1TypeParameters.size(), c2TypeParameters.size()); ++i3) {
                ...
                for (int j2 = 0; j2 < c1Bounds.size(); ++j2) {
                    String b2;
                    String b1 = RENDERER.renderType(c1Bounds.get(j2));
                    int boundCompareTo = b1.compareTo(b2 = RENDERER.renderType(c2Bounds.get(j2)));
