...
public final class CodedInputStream {
                ...
                block8: {
                    block7: {
                        block5: {
                            ...
                            if ((long)(x2 ^= buffer[pos++] << 7) >= 0L) break block5;
                            x2 = (int)((long)x2 ^ 0xFFFFFFFFFFFFFF80L);
                            break block6;
                        }
                        if ((long)(x2 ^= buffer[pos++] << 14) < 0L) break block7;
                        x2 = (int)((long)x2 ^ 0x3F80L);
                        break block6;
                    }
                    if ((long)(x2 ^= buffer[pos++] << 21) >= 0L) break block8;
                    x2 = (int)((long)x2 ^ 0xFFFFFFFFFFE03F80L);
                    break block6;
                }
                byte y2 = buffer[pos++];
                x2 ^= y2 << 28;
                x2 = (int)((long)x2 ^ 0xFE03F80L);
                if (y2 < 0 && buffer[pos++] < 0 && buffer[pos++] < 0 && buffer[pos++] < 0 && buffer[pos++] < 0 && buffer[pos++] < 0) break block4;
            }
            this.bufferPos = pos;
            return x2;
        }
                                ...
                                block8: {
                                    block7: {
                                        block5: {
                                            ...
                                            if (this.bufferSize - pos < 9) break block4;
                                            if ((x2 = (long)(y2 ^ buffer[pos++] << 7)) >= 0L) break block5;
                                            x2 ^= 0xFFFFFFFFFFFFFF80L;
                                            break block6;
                                        }
                                        if ((x2 ^= (long)(buffer[pos++] << 14)) < 0L) break block7;
                                        x2 ^= 0x3F80L;
                                        break block6;
                                    }
                                    if ((x2 ^= (long)(buffer[pos++] << 21)) >= 0L) break block8;
                                    x2 ^= 0xFFFFFFFFFFE03F80L;
                                    break block6;
                                }
                                if ((x2 ^= (long)buffer[pos++] << 28) < 0L) break block9;
                                x2 ^= 0xFE03F80L;
                                break block6;
                            }
                            if ((x2 ^= (long)buffer[pos++] << 35) >= 0L) break block10;
                            x2 ^= 0xFFFFFFF80FE03F80L;
                            break block6;
                        }
                        if ((x2 ^= (long)buffer[pos++] << 42) < 0L) break block11;
                        x2 ^= 0x3F80FE03F80L;
                        break block6;
                    }
                    if ((x2 ^= (long)buffer[pos++] << 49) >= 0L) break block12;
                    x2 ^= 0xFFFE03F80FE03F80L;
                    break block6;
                }
                x2 ^= (long)buffer[pos++] << 56;
                if ((x2 ^= 0xFE03F80FE03F80L) < 0L && (long)buffer[pos++] < 0L) break block4;
            }
            this.bufferPos = pos;
            return x2;
        }
    ...
    private void ensureAvailable(int n2) throws IOException {
        if (this.bufferSize - this.bufferPos < n2) {
    ...
    private boolean tryRefillBuffer(int n2) throws IOException {
        if (this.bufferPos + n2 <= this.bufferSize) {
            int n3 = n2;
            throw new IllegalStateException(new StringBuilder(77).append("refillBuffer() called when ").append(n3).append(" bytes were already available in buffer").toString());
        }
        ...
        if (this.input != null) {
            ...
            if ((bytesRead = this.input.read(this.buffer, this.bufferSize, this.buffer.length - this.bufferSize)) == 0 || bytesRead < -1 || bytesRead > this.buffer.length) {
                int n4 = bytesRead;
                throw new IllegalStateException(new StringBuilder(102).append("InputStream#read(byte[]) returned invalid result: ").append(n4).append("\nThe InputStream implementation is buggy.").toString());
            }
            if (bytesRead > 0) {
    ...
    private byte[] readRawBytesSlowPath(int size) throws IOException {
        ...
        for (int sizeLeft = size - (originalBufferSize - originalBufferPos); sizeLeft > 0; sizeLeft -= chunk.length) {
            ...
            for (pos = 0; pos < chunk.length; pos += n2) {
                int n3 = n2 = this.input == null ? -1 : this.input.read(chunk, pos, chunk.length - pos);
                if (n2 == -1) {
                    throw InvalidProtocolBufferException.truncatedMessage();
                }
