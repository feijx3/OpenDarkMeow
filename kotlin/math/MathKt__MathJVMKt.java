/*
 * Decompiled with CFR 0.152.
 * 
 * Could not load the following classes:
 *  kotlin.ExperimentalStdlibApi
 *  kotlin.SinceKotlin
 *  kotlin.WasExperimental
 *  kotlin.internal.InlineOnly
 */
package kotlin.math;

import kotlin.ExperimentalStdlibApi;
import kotlin.Metadata;
import kotlin.SinceKotlin;
import kotlin.WasExperimental;
import kotlin.internal.InlineOnly;
import kotlin.math.Constants;
import kotlin.math.MathKt;
import kotlin.math.MathKt__MathHKt;

@Metadata(mv={2, 2, 0}, k=5, xi=49, d1={"\u0000\u001e\n\u0000\n\u0002\u0010\u0006\n\u0002\b%\n\u0002\u0010\b\n\u0002\b\u0013\n\u0002\u0010\t\n\u0002\u0010\u0007\n\u0002\b\b\u001a\u0011\u0010\u0000\u001a\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u0001H\u0087\b\u001a\u0011\u0010\u0003\u001a\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u0001H\u0087\b\u001a\u0011\u0010\u0004\u001a\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u0001H\u0087\b\u001a\u0011\u0010\u0005\u001a\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u0001H\u0087\b\u001a\u0011\u0010\u0006\u001a\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u0001H\u0087\b\u001a\u0011\u0010\u0007\u001a\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u0001H\u0087\b\u001a\u0019\u0010\b\u001a\u00020\u00012\u0006\u0010\t\u001a\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u0001H\u0087\b\u001a\u0011\u0010\n\u001a\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u0001H\u0087\b\u001a\u0011\u0010\u000b\u001a\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u0001H\u0087\b\u001a\u0011\u0010\f\u001a\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u0001H\u0087\b\u001a\u0010\u0010\r\u001a\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u0001H\u0007\u001a\u0010\u0010\u000e\u001a\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u0001H\u0007\u001a\u0010\u0010\u000f\u001a\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u0001H\u0007\u001a\u0019\u0010\u0010\u001a\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u00012\u0006\u0010\t\u001a\u00020\u0001H\u0087\b\u001a\u0011\u0010\u0011\u001a\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u0001H\u0087\b\u001a\u0011\u0010\u0012\u001a\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u0001H\u0087\b\u001a\u0011\u0010\u0013\u001a\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u0001H\u0087\b\u001a\u0018\u0010\u0014\u001a\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u00012\u0006\u0010\u0015\u001a\u00020\u0001H\u0007\u001a\u0011\u0010\u0016\u001a\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u0001H\u0087\b\u001a\u0011\u0010\u0017\u001a\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u0001H\u0087\b\u001a\u0010\u0010\u0018\u001a\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u0001H\u0007\u001a\u0011\u0010\u0019\u001a\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u0001H\u0087\b\u001a\u0011\u0010\u001a\u001a\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u0001H\u0087\b\u001a\u0011\u0010\u001b\u001a\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u0001H\u0087\b\u001a\u0010\u0010\u001c\u001a\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u0001H\u0007\u001a\u0011\u0010\u001d\u001a\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u0001H\u0087\b\u001a\u0011\u0010\u001e\u001a\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u0001H\u0087\b\u001a\u0011\u0010\u001f\u001a\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u0001H\u0087\b\u001a\u0019\u0010 \u001a\u00020\u00012\u0006\u0010!\u001a\u00020\u00012\u0006\u0010\"\u001a\u00020\u0001H\u0087\b\u001a\u0019\u0010#\u001a\u00020\u00012\u0006\u0010!\u001a\u00020\u00012\u0006\u0010\"\u001a\u00020\u0001H\u0087\b\u001a\u0011\u0010$\u001a\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u0001H\u0087\b\u001a\u0015\u0010%\u001a\u00020\u0001*\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u0001H\u0087\b\u001a\u0015\u0010%\u001a\u00020\u0001*\u00020\u00012\u0006\u0010&\u001a\u00020'H\u0087\b\u001a\u0015\u0010(\u001a\u00020\u0001*\u00020\u00012\u0006\u0010)\u001a\u00020\u0001H\u0087\b\u001a\u0015\u00101\u001a\u00020\u0001*\u00020\u00012\u0006\u0010\u001f\u001a\u00020\u0001H\u0087\b\u001a\u0015\u00101\u001a\u00020\u0001*\u00020\u00012\u0006\u0010\u001f\u001a\u00020'H\u0087\b\u001a\r\u00105\u001a\u00020\u0001*\u00020\u0001H\u0087\b\u001a\r\u00106\u001a\u00020\u0001*\u00020\u0001H\u0087\b\u001a\u0015\u00107\u001a\u00020\u0001*\u00020\u00012\u0006\u00108\u001a\u00020\u0001H\u0087\b\u001a\f\u00109\u001a\u00020'*\u00020\u0001H\u0007\u001a\f\u0010:\u001a\u00020;*\u00020\u0001H\u0007\u001a\u0011\u0010\u0000\u001a\u00020<2\u0006\u0010\u0002\u001a\u00020<H\u0087\b\u001a\u0011\u0010\u0003\u001a\u00020<2\u0006\u0010\u0002\u001a\u00020<H\u0087\b\u001a\u0011\u0010\u0004\u001a\u00020<2\u0006\u0010\u0002\u001a\u00020<H\u0087\b\u001a\u0011\u0010\u0005\u001a\u00020<2\u0006\u0010\u0002\u001a\u00020<H\u0087\b\u001a\u0011\u0010\u0006\u001a\u00020<2\u0006\u0010\u0002\u001a\u00020<H\u0087\b\u001a\u0011\u0010\u0007\u001a\u00020<2\u0006\u0010\u0002\u001a\u00020<H\u0087\b\u001a\u0019\u0010\b\u001a\u00020<2\u0006\u0010\t\u001a\u00020<2\u0006\u0010\u0002\u001a\u00020<H\u0087\b\u001a\u0011\u0010\n\u001a\u00020<2\u0006\u0010\u0002\u001a\u00020<H\u0087\b\u001a\u0011\u0010\u000b\u001a\u00020<2\u0006\u0010\u0002\u001a\u00020<H\u0087\b\u001a\u0011\u0010\f\u001a\u00020<2\u0006\u0010\u0002\u001a\u00020<H\u0087\b\u001a\u0011\u0010\r\u001a\u00020<2\u0006\u0010\u0002\u001a\u00020<H\u0087\b\u001a\u0011\u0010\u000e\u001a\u00020<2\u0006\u0010\u0002\u001a\u00020<H\u0087\b\u001a\u0011\u0010\u000f\u001a\u00020<2\u0006\u0010\u0002\u001a\u00020<H\u0087\b\u001a\u0019\u0010\u0010\u001a\u00020<2\u0006\u0010\u0002\u001a\u00020<2\u0006\u0010\t\u001a\u00020<H\u0087\b\u001a\u0011\u0010\u0011\u001a\u00020<2\u0006\u0010\u0002\u001a\u00020<H\u0087\b\u001a\u0011\u0010\u0012\u001a\u00020<2\u0006\u0010\u0002\u001a\u00020<H\u0087\b\u001a\u0011\u0010\u0013\u001a\u00020<2\u0006\u0010\u0002\u001a\u00020<H\u0087\b\u001a\u0018\u0010\u0014\u001a\u00020<2\u0006\u0010\u0002\u001a\u00020<2\u0006\u0010\u0015\u001a\u00020<H\u0007\u001a\u0011\u0010\u0016\u001a\u00020<2\u0006\u0010\u0002\u001a\u00020<H\u0087\b\u001a\u0011\u0010\u0017\u001a\u00020<2\u0006\u0010\u0002\u001a\u00020<H\u0087\b\u001a\u0010\u0010\u0018\u001a\u00020<2\u0006\u0010\u0002\u001a\u00020<H\u0007\u001a\u0011\u0010\u0019\u001a\u00020<2\u0006\u0010\u0002\u001a\u00020<H\u0087\b\u001a\u0011\u0010\u001a\u001a\u00020<2\u0006\u0010\u0002\u001a\u00020<H\u0087\b\u001a\u0011\u0010\u001b\u001a\u00020<2\u0006\u0010\u0002\u001a\u00020<H\u0087\b\u001a\u0010\u0010\u001c\u001a\u00020<2\u0006\u0010\u0002\u001a\u00020<H\u0007\u001a\u0011\u0010\u001d\u001a\u00020<2\u0006\u0010\u0002\u001a\u00020<H\u0087\b\u001a\u0011\u0010\u001e\u001a\u00020<2\u0006\u0010\u0002\u001a\u00020<H\u0087\b\u001a\u0011\u0010\u001f\u001a\u00020<2\u0006\u0010\u0002\u001a\u00020<H\u0087\b\u001a\u0019\u0010 \u001a\u00020<2\u0006\u0010!\u001a\u00020<2\u0006\u0010\"\u001a\u00020<H\u0087\b\u001a\u0019\u0010#\u001a\u00020<2\u0006\u0010!\u001a\u00020<2\u0006\u0010\"\u001a\u00020<H\u0087\b\u001a\u0011\u0010$\u001a\u00020<2\u0006\u0010\u0002\u001a\u00020<H\u0087\b\u001a\u0015\u0010%\u001a\u00020<*\u00020<2\u0006\u0010\u0002\u001a\u00020<H\u0087\b\u001a\u0015\u0010%\u001a\u00020<*\u00020<2\u0006\u0010&\u001a\u00020'H\u0087\b\u001a\u0015\u0010(\u001a\u00020<*\u00020<2\u0006\u0010)\u001a\u00020<H\u0087\b\u001a\u0015\u00101\u001a\u00020<*\u00020<2\u0006\u0010\u001f\u001a\u00020<H\u0087\b\u001a\u0015\u00101\u001a\u00020<*\u00020<2\u0006\u0010\u001f\u001a\u00020'H\u0087\b\u001a\r\u00105\u001a\u00020<*\u00020<H\u0087\b\u001a\r\u00106\u001a\u00020<*\u00020<H\u0087\b\u001a\u0015\u00107\u001a\u00020<*\u00020<2\u0006\u00108\u001a\u00020<H\u0087\b\u001a\f\u00109\u001a\u00020'*\u00020<H\u0007\u001a\f\u0010:\u001a\u00020;*\u00020<H\u0007\u001a\u0011\u0010\u001e\u001a\u00020'2\u0006\u0010&\u001a\u00020'H\u0087\b\u001a\u0019\u0010 \u001a\u00020'2\u0006\u0010!\u001a\u00020'2\u0006\u0010\"\u001a\u00020'H\u0087\b\u001a\u0019\u0010#\u001a\u00020'2\u0006\u0010!\u001a\u00020'2\u0006\u0010\"\u001a\u00020'H\u0087\b\u001a\u0011\u0010\u001e\u001a\u00020;2\u0006\u0010&\u001a\u00020;H\u0087\b\u001a\u0019\u0010 \u001a\u00020;2\u0006\u0010!\u001a\u00020;2\u0006\u0010\"\u001a\u00020;H\u0087\b\u001a\u0019\u0010#\u001a\u00020;2\u0006\u0010!\u001a\u00020;2\u0006\u0010\"\u001a\u00020;H\u0087\b\"\u001f\u0010*\u001a\u00020\u0001*\u00020\u00018\u00c6\u0002X\u0087\u0004\u00a2\u0006\f\u0012\u0004\b+\u0010,\u001a\u0004\b-\u0010.\"\u001f\u0010\u001f\u001a\u00020\u0001*\u00020\u00018\u00c6\u0002X\u0087\u0004\u00a2\u0006\f\u0012\u0004\b/\u0010,\u001a\u0004\b0\u0010.\"\u001f\u00102\u001a\u00020\u0001*\u00020\u00018\u00c6\u0002X\u0087\u0004\u00a2\u0006\f\u0012\u0004\b3\u0010,\u001a\u0004\b4\u0010.\"\u001f\u0010*\u001a\u00020<*\u00020<8\u00c6\u0002X\u0087\u0004\u00a2\u0006\f\u0012\u0004\b+\u0010=\u001a\u0004\b-\u0010>\"\u001f\u0010\u001f\u001a\u00020<*\u00020<8\u00c6\u0002X\u0087\u0004\u00a2\u0006\f\u0012\u0004\b/\u0010=\u001a\u0004\b0\u0010>\"\u001f\u00102\u001a\u00020<*\u00020<8\u00c6\u0002X\u0087\u0004\u00a2\u0006\f\u0012\u0004\b3\u0010=\u001a\u0004\b4\u0010>\"\u001f\u0010*\u001a\u00020'*\u00020'8\u00c6\u0002X\u0087\u0004\u00a2\u0006\f\u0012\u0004\b+\u0010?\u001a\u0004\b-\u0010@\"\u001e\u0010\u001f\u001a\u00020'*\u00020'8FX\u0087\u0004\u00a2\u0006\f\u0012\u0004\b/\u0010?\u001a\u0004\b0\u0010@\"\u001f\u0010*\u001a\u00020;*\u00020;8\u00c6\u0002X\u0087\u0004\u00a2\u0006\f\u0012\u0004\b+\u0010A\u001a\u0004\b-\u0010B\"\u001e\u0010\u001f\u001a\u00020'*\u00020;8FX\u0087\u0004\u00a2\u0006\f\u0012\u0004\b/\u0010A\u001a\u0004\b0\u0010C\u00a8\u0006D"}, d2={"sin", "", "x", "cos", "tan", "asin", "acos", "atan", "atan2", "y", "sinh", "cosh", "tanh", "asinh", "acosh", "atanh", "hypot", "sqrt", "exp", "expm1", "log", "base", "ln", "log10", "log2", "ln1p", "ceil", "floor", "truncate", "round", "abs", "sign", "min", "a", "b", "max", "cbrt", "pow", "n", "", "IEEErem", "divisor", "absoluteValue", "getAbsoluteValue$annotations", "(D)V", "getAbsoluteValue", "(D)D", "getSign$annotations", "getSign", "withSign", "ulp", "getUlp$annotations", "getUlp", "nextUp", "nextDown", "nextTowards", "to", "roundToInt", "roundToLong", "", "", "(F)V", "(F)F", "(I)V", "(I)I", "(J)V", "(J)J", "(J)I", "kotlin-stdlib"}, xs="kotlin/math/MathKt")
class MathKt__MathJVMKt
extends MathKt__MathHKt {
    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double sin(double x2) {
        return Math.sin(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double cos(double x2) {
        return Math.cos(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double tan(double x2) {
        return Math.tan(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double asin(double x2) {
        return Math.asin(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double acos(double x2) {
        return Math.acos(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double atan(double x2) {
        return Math.atan(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double atan2(double y2, double x2) {
        return Math.atan2(y2, x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double sinh(double x2) {
        return Math.sinh(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double cosh(double x2) {
        return Math.cosh(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double tanh(double x2) {
        return Math.tanh(x2);
    }

    /*
     * WARNING - void declaration
     */
    @SinceKotlin(version="1.2")
    public static final double asinh(double x2) {
        double d2;
        if (x2 >= Constants.taylor_n_bound) {
            d2 = x2 > Constants.upper_taylor_n_bound ? (x2 > Constants.upper_taylor_2_bound ? Math.log(x2) + Constants.LN2 : Math.log(x2 * (double)2 + 1.0 / (x2 * (double)2))) : Math.log(x2 + Math.sqrt(x2 * x2 + 1.0));
        } else if (x2 <= -Constants.taylor_n_bound) {
            d2 = -MathKt.asinh(-x2);
        } else {
            void var2_1;
            double result = x2;
            if (Math.abs(x2) >= Constants.taylor_2_bound) {
                result -= x2 * x2 * x2 / (double)6;
            }
            d2 = var2_1;
        }
        return d2;
    }

    @SinceKotlin(version="1.2")
    public static final double acosh(double x2) {
        double d2;
        if (x2 < 1.0) {
            d2 = Double.NaN;
        } else if (x2 > Constants.upper_taylor_2_bound) {
            d2 = Math.log(x2) + Constants.LN2;
        } else if (x2 - 1.0 >= Constants.taylor_n_bound) {
            d2 = Math.log(x2 + Math.sqrt(x2 * x2 - 1.0));
        } else {
            double y2;
            double result = y2 = Math.sqrt(x2 - 1.0);
            if (y2 >= Constants.taylor_2_bound) {
                result -= y2 * y2 * y2 / (double)12;
            }
            d2 = Math.sqrt(2.0) * result;
        }
        return d2;
    }

    @SinceKotlin(version="1.2")
    public static final double atanh(double x2) {
        if (Math.abs(x2) < Constants.taylor_n_bound) {
            double result = x2;
            if (Math.abs(x2) > Constants.taylor_2_bound) {
                result += x2 * x2 * x2 / (double)3;
            }
            return result;
        }
        return Math.log((1.0 + x2) / (1.0 - x2)) / (double)2;
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double hypot(double x2, double y2) {
        return Math.hypot(x2, y2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double sqrt(double x2) {
        return Math.sqrt(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double exp(double x2) {
        return Math.exp(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double expm1(double x2) {
        return Math.expm1(x2);
    }

    @SinceKotlin(version="1.2")
    public static final double log(double x2, double base) {
        if (base <= 0.0 || base == 1.0) {
            return Double.NaN;
        }
        return Math.log(x2) / Math.log(base);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double ln(double x2) {
        return Math.log(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double log10(double x2) {
        return Math.log10(x2);
    }

    @SinceKotlin(version="1.2")
    public static final double log2(double x2) {
        return Math.log(x2) / Constants.LN2;
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double ln1p(double x2) {
        return Math.log1p(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double ceil(double x2) {
        return Math.ceil(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double floor(double x2) {
        return Math.floor(x2);
    }

    @SinceKotlin(version="1.2")
    public static final double truncate(double x2) {
        return Double.isNaN(x2) || Double.isInfinite(x2) ? x2 : (x2 > 0.0 ? Math.floor(x2) : Math.ceil(x2));
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double round(double x2) {
        return Math.rint(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double abs(double x2) {
        return Math.abs(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double sign(double x2) {
        return Math.signum(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double min(double a2, double b2) {
        return Math.min(a2, b2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double max(double a2, double b2) {
        return Math.max(a2, b2);
    }

    @SinceKotlin(version="1.8")
    @WasExperimental(markerClass={ExperimentalStdlibApi.class})
    @InlineOnly
    private static final double cbrt(double x2) {
        return Math.cbrt(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double pow(double $this$pow, double x2) {
        return Math.pow($this$pow, x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double pow(double $this$pow, int n2) {
        return Math.pow($this$pow, n2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double IEEErem(double $this$IEEErem, double divisor) {
        return Math.IEEEremainder($this$IEEErem, divisor);
    }

    private static final double getAbsoluteValue(double $this$absoluteValue) {
        return Math.abs($this$absoluteValue);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    public static /* synthetic */ void getAbsoluteValue$annotations(double d2) {
    }

    private static final double getSign(double $this$sign) {
        return Math.signum($this$sign);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    public static /* synthetic */ void getSign$annotations(double d2) {
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double withSign(double $this$withSign, double sign) {
        return Math.copySign($this$withSign, sign);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double withSign(double $this$withSign, int sign) {
        return Math.copySign($this$withSign, (double)sign);
    }

    private static final double getUlp(double $this$ulp) {
        return Math.ulp($this$ulp);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    public static /* synthetic */ void getUlp$annotations(double d2) {
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double nextUp(double $this$nextUp) {
        return Math.nextUp($this$nextUp);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double nextDown(double $this$nextDown) {
        return Math.nextAfter($this$nextDown, Double.NEGATIVE_INFINITY);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final double nextTowards(double $this$nextTowards, double to) {
        return Math.nextAfter($this$nextTowards, to);
    }

    @SinceKotlin(version="1.2")
    public static final int roundToInt(double $this$roundToInt) {
        if (Double.isNaN($this$roundToInt)) {
            throw new IllegalArgumentException("Cannot round NaN value.");
        }
        return $this$roundToInt > 2.147483647E9 ? Integer.MAX_VALUE : ($this$roundToInt < -2.147483648E9 ? Integer.MIN_VALUE : (int)Math.round($this$roundToInt));
    }

    @SinceKotlin(version="1.2")
    public static final long roundToLong(double $this$roundToLong) {
        if (Double.isNaN($this$roundToLong)) {
            throw new IllegalArgumentException("Cannot round NaN value.");
        }
        return Math.round($this$roundToLong);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float sin(float x2) {
        return (float)Math.sin(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float cos(float x2) {
        return (float)Math.cos(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float tan(float x2) {
        return (float)Math.tan(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float asin(float x2) {
        return (float)Math.asin(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float acos(float x2) {
        return (float)Math.acos(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float atan(float x2) {
        return (float)Math.atan(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float atan2(float y2, float x2) {
        return (float)Math.atan2(y2, x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float sinh(float x2) {
        return (float)Math.sinh(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float cosh(float x2) {
        return (float)Math.cosh(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float tanh(float x2) {
        return (float)Math.tanh(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float asinh(float x2) {
        return (float)MathKt.asinh((double)x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float acosh(float x2) {
        return (float)MathKt.acosh((double)x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float atanh(float x2) {
        return (float)MathKt.atanh((double)x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float hypot(float x2, float y2) {
        return (float)Math.hypot(x2, y2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float sqrt(float x2) {
        return (float)Math.sqrt(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float exp(float x2) {
        return (float)Math.exp(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float expm1(float x2) {
        return (float)Math.expm1(x2);
    }

    @SinceKotlin(version="1.2")
    public static final float log(float x2, float base) {
        if (base <= 0.0f || base == 1.0f) {
            return Float.NaN;
        }
        return (float)(Math.log(x2) / Math.log(base));
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float ln(float x2) {
        return (float)Math.log(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float log10(float x2) {
        return (float)Math.log10(x2);
    }

    @SinceKotlin(version="1.2")
    public static final float log2(float x2) {
        return (float)(Math.log(x2) / Constants.LN2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float ln1p(float x2) {
        return (float)Math.log1p(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float ceil(float x2) {
        return (float)Math.ceil(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float floor(float x2) {
        return (float)Math.floor(x2);
    }

    @SinceKotlin(version="1.2")
    public static final float truncate(float x2) {
        return Float.isNaN(x2) || Float.isInfinite(x2) ? x2 : (x2 > 0.0f ? (float)Math.floor(x2) : (float)Math.ceil(x2));
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float round(float x2) {
        return (float)Math.rint(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float abs(float x2) {
        return Math.abs(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float sign(float x2) {
        return Math.signum(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float min(float a2, float b2) {
        return Math.min(a2, b2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float max(float a2, float b2) {
        return Math.max(a2, b2);
    }

    @SinceKotlin(version="1.8")
    @WasExperimental(markerClass={ExperimentalStdlibApi.class})
    @InlineOnly
    private static final float cbrt(float x2) {
        return (float)Math.cbrt(x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float pow(float $this$pow, float x2) {
        return (float)Math.pow($this$pow, x2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float pow(float $this$pow, int n2) {
        return (float)Math.pow($this$pow, n2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float IEEErem(float $this$IEEErem, float divisor) {
        return (float)Math.IEEEremainder($this$IEEErem, divisor);
    }

    private static final float getAbsoluteValue(float $this$absoluteValue) {
        return Math.abs($this$absoluteValue);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    public static /* synthetic */ void getAbsoluteValue$annotations(float f2) {
    }

    private static final float getSign(float $this$sign) {
        return Math.signum($this$sign);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    public static /* synthetic */ void getSign$annotations(float f2) {
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float withSign(float $this$withSign, float sign) {
        return Math.copySign($this$withSign, sign);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float withSign(float $this$withSign, int sign) {
        return Math.copySign($this$withSign, sign);
    }

    private static final float getUlp(float $this$ulp) {
        return Math.ulp($this$ulp);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    public static /* synthetic */ void getUlp$annotations(float f2) {
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float nextUp(float $this$nextUp) {
        return Math.nextUp($this$nextUp);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float nextDown(float $this$nextDown) {
        return Math.nextAfter($this$nextDown, Double.NEGATIVE_INFINITY);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final float nextTowards(float $this$nextTowards, float to) {
        return Math.nextAfter($this$nextTowards, (double)to);
    }

    @SinceKotlin(version="1.2")
    public static final int roundToInt(float $this$roundToInt) {
        if (Float.isNaN($this$roundToInt)) {
            throw new IllegalArgumentException("Cannot round NaN value.");
        }
        return Math.round($this$roundToInt);
    }

    @SinceKotlin(version="1.2")
    public static final long roundToLong(float $this$roundToLong) {
        return MathKt.roundToLong((double)$this$roundToLong);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final int abs(int n2) {
        return Math.abs(n2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final int min(int a2, int b2) {
        return Math.min(a2, b2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final int max(int a2, int b2) {
        return Math.max(a2, b2);
    }

    private static final int getAbsoluteValue(int $this$absoluteValue) {
        return Math.abs($this$absoluteValue);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    public static /* synthetic */ void getAbsoluteValue$annotations(int n2) {
    }

    public static final int getSign(int $this$sign) {
        return Integer.signum($this$sign);
    }

    @SinceKotlin(version="1.2")
    public static /* synthetic */ void getSign$annotations(int n2) {
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final long abs(long n2) {
        return Math.abs(n2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final long min(long a2, long b2) {
        return Math.min(a2, b2);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    private static final long max(long a2, long b2) {
        return Math.max(a2, b2);
    }

    private static final long getAbsoluteValue(long $this$absoluteValue) {
        return Math.abs($this$absoluteValue);
    }

    @SinceKotlin(version="1.2")
    @InlineOnly
    public static /* synthetic */ void getAbsoluteValue$annotations(long l2) {
    }

    public static final int getSign(long $this$sign) {
        return Long.signum($this$sign);
    }

    @SinceKotlin(version="1.2")
    public static /* synthetic */ void getSign$annotations(long l2) {
    }
}

