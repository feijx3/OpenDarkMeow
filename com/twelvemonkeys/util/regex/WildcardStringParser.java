...
public class WildcardStringParser {
    ...
    public WildcardStringParser(String string, boolean bl2) {
        this(string, bl2, System.out);
    }

    public WildcardStringParser(String string, boolean bl2, PrintStream printStream) {
    ...
    private boolean isTrivialAutomaton() {
        for (int i2 = 0; i2 < this.stringMask.length(); ++i2) {
            if (WildcardStringParser.isFreeRangeCharacter(this.stringMask.charAt(i2))) continue;
            return false;
        }
        return true;
    }
    ...
    private boolean buildAutomaton() {
        ...
        for (int i2 = 1; i2 < this.stringMask.length(); ++i2) {
            char c2 = this.stringMask.charAt(i2);
            if (!WildcardStringParser.isInAlphabet(c2) && !WildcardStringParser.isWildcardCharacter(c2)) {
            ...
            wildcardStringParserState2 = new WildcardStringParserState(c2);
            wildcardStringParserState2.automatonStateNumber = i2;
    ...
    public static boolean isInAlphabet(char c2) {
        for (int i2 = 0; i2 < ALPHABET.length; ++i2) {
            if (c2 != ALPHABET[i2]) continue;
            return true;
        }
        return false;
    }

    public static boolean isFreeRangeCharacter(char c2) {
        return c2 == '*';
    }

    public static boolean isFreePassCharacter(char c2) {
        return c2 == '?';
    }

    public static boolean isWildcardCharacter(char c2) {
        return WildcardStringParser.isFreeRangeCharacter(c2) || WildcardStringParser.isFreePassCharacter(c2);
    }
    ...
    public boolean parseString(String string) {
        ...
        for (int i2 = 0; i2 < parsableString.length(); ++i2) {
            if (this.debugging) {
                this.out.println();
            }
            if (this.debugging) {
                this.out.println("parsing - index number " + i2 + ", active char: '" + parsableString.getActiveChar() + "' char string index: " + parsableString.index + " number of chars since last free-range state: " + n3);
            }
            if (this.debugging) {
                this.out.println("parsing - state: " + wildcardStringParserState.automatonStateNumber + " '" + wildcardStringParserState.character + "' - no of free-pass chars read: " + n2);
            }
            if (this.debugging) {
                this.out.println("parsing - hasPerformedFreeRangeMovement: " + bl2);
            }
        ...
        boolean checkString() {
            if (!this.isEmpty()) {
                for (int i2 = 0; i2 < this.charArray.length; ++i2) {
                    if (WildcardStringParser.isInAlphabet(this.charArray[i2])) continue;
                    return false;
                }
            }
            return true;
    ...
    class WildcardStringParserState {
        ...
        public WildcardStringParserState(char c2) {
            this.character = c2;
        }
    }
}
