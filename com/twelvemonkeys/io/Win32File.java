...
extends File {
    ...
    public static void main(String[] stringArray) {
        int n2 = 0;
        boolean bl2 = false;
        while (stringArray.length > n2 + 1 && stringArray[n2].charAt(0) == '-' && stringArray[n2].length() > 1) {
            if (stringArray[n2].charAt(1) == 'R' || stringArray[n2].equals("--recursive")) {
                bl2 = true;
            } else {
        ...
        File file = Win32File.wrap(new File(stringArray[n2]));
        System.out.println("file: " + file);
        System.out.println("file.getClass(): " + file.getClass());
        Win32File.listFiles(file, 0, bl2);
    }

    private static void listFiles(File file, int n2, boolean bl2) {
        if (file.isDirectory()) {
            ...
            for (File file2 : fileArray) {
                ...
                if (!bl2) continue;
                Win32File.listFiles(file2, n2 + 1, n2 < 4);
            }
        }
    }
    ...
    public static File[] wrap(File[] fileArray) {
        if (IS_WINDOWS) {
            for (int i2 = 0; fileArray != null && i2 < fileArray.length; ++i2) {
                fileArray[i2] = Win32File.wrap(fileArray[i2]);
            }
        }
        return fileArray;
    }
