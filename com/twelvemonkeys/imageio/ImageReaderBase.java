...
extends ImageReader {
    ...
    protected void checkBounds(int n2) throws IOException {
        this.assertInput();
        if (n2 < this.getMinIndex()) {
            throw new IndexOutOfBoundsException("index < minIndex");
        }
        int n3 = this.getNumImages(false);
        if (n3 != -1 && n2 >= n3) {
            throw new IndexOutOfBoundsException("index >= numImages (" + n2 + " >= " + n3 + ")");
        }
    }
    ...
    public static BufferedImage getDestination(ImageReadParam imageReadParam, Iterator<ImageTypeSpecifier> iterator2, int n2, int n3) throws IIOException {
        Object object;
        Object object2;
        ...
        if (imageReadParam != null) {
            ...
            if (object2 != null) {
                ...
                while (iterator2.hasNext()) {
                    ...
                    int n4 = imageTypeSpecifier2.getBufferedImageType();
                    if (n4 != 0 && n4 == ((BufferedImage)object2).getType()) {
                        bl2 = true;
                        break;
                    }
        ...
        if (imageTypeSpecifier == null) {
            imageTypeSpecifier = iterator2.next();
        } else {
            boolean bl3 = false;
            while (iterator2.hasNext()) {
        ...
        ImageReaderBase.computeRegions(imageReadParam, n2, n3, null, (Rectangle)object2, (Rectangle)object);
        int n5 = ((Rectangle)object).x + ((Rectangle)object).width;
        int n6 = ((Rectangle)object).y + ((Rectangle)object).height;
        long l2 = (long)n5 * (long)n6;
        if (l2 > Integer.MAX_VALUE) {
            throw new IIOException(String.format("destination width * height > Integer.MAX_VALUE: %d", l2));
        }
        long l3 = l2 * (long)imageTypeSpecifier.getSampleModel().getNumDataElements();
        if (l3 > Integer.MAX_VALUE) {
            throw new IIOException(String.format("destination width * height * samplesPerPixel > Integer.MAX_VALUE: %d", l3));
        }
        return imageTypeSpecifier.createBufferedImage(n5, n6);
    }
    ...
    protected static Image fakeSubsampling(Image image2, ImageReadParam imageReadParam) {
        return IIOUtil.fakeSubsampling(image2, imageReadParam);
    }
    ...
    extends JLabel {
            ...
            this.addMouseListener(new MouseAdapter(){
                ...
                public void mouseClicked(MouseEvent mouseEvent) {
                    if (mouseEvent.isPopupTrigger()) {
                        this.getComponentPopupMenu().show(this, mouseEvent.getX(), mouseEvent.getY());
                    }
                }
            });
            this.setTransferHandler(new TransferHandler(){
                ...
                public boolean importData(JComponent jComponent, Transferable transferable) {
                    if (this.canImport(jComponent, transferable.getTransferDataFlavors())) {
                        try {
                            Image image2 = (Image)transferable.getTransferData(DataFlavor.imageFlavor);
                            image = ImageUtil.toBuffered(image2);
                            this.setIcon(new BufferedImageIcon(image));
                            return true;
                        }
        ...
        private static Color getDefaultBackground(BufferedImage bufferedImage) {
            ...
            int n2;
            if (bufferedImage.getColorModel() instanceof IndexColorModel && (n2 = (indexColorModel = (IndexColorModel)bufferedImage.getColorModel()).getTransparentPixel()) >= 0) {
                return new Color(indexColorModel.getRGB(n2), false);
            }
            return null;
        }
        ...
        extends ZoomAction {
            ...
            public void actionPerformed(ActionEvent actionEvent) {
                ...
                double d2 = (double)container.getWidth() / (double)ImageLabel.this.image.getWidth();
                double d3 = (double)container.getHeight() / (double)ImageLabel.this.image.getHeight();
                double d4 = Math.min(d2, d3);
                int n2 = Math.max(Math.min((int)((double)ImageLabel.this.image.getWidth() * d4), ImageLabel.this.image.getWidth() * 16), ImageLabel.this.image.getWidth() / 16);
                int n3 = Math.max(Math.min((int)((double)ImageLabel.this.image.getHeight() * d4), ImageLabel.this.image.getHeight() * 16), ImageLabel.this.image.getHeight() / 16);
                ImageLabel.this.setIcon(new BufferedImageIcon(ImageLabel.this.image, n2, n3, d4 > 1.0));
            }
        }
        ...
        extends AbstractAction {
            ...
            public ZoomAction(String string, double d2) {
                super(string);
                this.zoomFactor = d2;
            }
            ...
            public void actionPerformed(ActionEvent actionEvent) {
                if (this.zoomFactor <= 0.0) {
                    ...
                    int n2 = Math.max(Math.min((int)((double)icon.getIconWidth() * this.zoomFactor), ImageLabel.this.image.getWidth() * 16), ImageLabel.this.image.getWidth() / 16);
                    int n3 = Math.max(Math.min((int)((double)icon.getIconHeight() * this.zoomFactor), ImageLabel.this.image.getHeight() * 16), ImageLabel.this.image.getHeight() / 16);
                    ImageLabel.this.setIcon(new BufferedImageIcon(ImageLabel.this.image, Math.max(n2, 2), Math.max(n3, 2), n2 > ImageLabel.this.image.getWidth() || n3 > ImageLabel.this.image.getHeight()));
                }
            }
        }
            ...
            public ChooseBackgroundAction(String string, Color color) {
                ...
                this.putValue("SmallIcon", new Icon(){
                    ...
                    public void paintIcon(Component component, Graphics graphics, int n2, int n3) {
                        ...
                        graphics2.fillRect(n2, n3, 16, 16);
                        graphics2.dispose();
                    }
