...
public final class ColorProfiles {
    ...
    public static ICC_Profile readProfile(InputStream inputStream) throws IOException {
        ...
        try {
            ...
            int n2 = ColorProfiles.validateHeaderAndGetSize(byArray);
            byte[] byArray2 = Arrays.copyOf(byArray, n2);
            dataInputStream.readFully(byArray2, byArray.length, n2 - byArray.length);
    ...
    public static ICC_Profile createProfileRaw(byte[] byArray) {
        int n2 = ColorProfiles.validateHeaderAndGetSize(byArray);
        return ICC_Profile.getInstance(ColorProfiles.limit(byArray, n2));
    }

    public static ICC_Profile createProfile(byte[] byArray) {
        int n2 = ColorProfiles.validateAndGetSize(byArray);
        ...
        ICC_Profile iCC_Profile2 = ICC_Profile.getInstance(ColorProfiles.limit(byArray, n2));
        return ColorSpaces.createColorSpace(iCC_Profile2).getProfile();
    }

    private static byte[] limit(byte[] byArray, int n2) {
        return byArray.length == n2 ? byArray : Arrays.copyOf(byArray, n2);
    }

    private static int validateAndGetSize(byte[] byArray) {
        int n2 = ColorProfiles.validateHeaderAndGetSize(byArray);
        if (n2 < 0 || n2 > byArray.length) {
            throw new IllegalArgumentException("Truncated ICC profile data, length < " + n2 + ": " + byArray.length);
        }
        return n2;
    }
    ...
    private static int validateHeaderAndGetSize(byte[] byArray) {
        ...
        int n2 = ColorProfiles.intBigEndian(byArray, 0);
    ...
    private static ICC_Profile getInternalProfile(byte[] byArray) {
        int n2 = ColorProfiles.getCsType(byArray);
        if (n2 == 5 && Arrays.equals(byArray, sRGB.header)) {
            return ICC_Profile.getInstance(1000);
        }
        if (n2 == 6 && Arrays.equals(byArray, GRAY.header)) {
            return ICC_Profile.getInstance(1003);
        }
        if (n2 == 13 && Arrays.equals(byArray, PYCC.header)) {
            return ICC_Profile.getInstance(1002);
        }
        if (n2 == 5 && Arrays.equals(byArray, LINEAR_RGB.header)) {
            return ICC_Profile.getInstance(1004);
        }
        if (n2 == 0 && Arrays.equals(byArray, CIEXYZ.header)) {
            return ICC_Profile.getInstance(1001);
        }
        return null;
    }

    private static int intBigEndian(byte[] byArray, int n2) {
    ...
    private static int getCsType(byte[] byArray) {
        int n2 = ColorProfiles.intBigEndian(byArray, 16);
        switch (n2) {
            case 1482250784: {
                return 0;
            }
        ...
        throw new IllegalArgumentException("Invalid ICC color space signature: " + n2);
    }
