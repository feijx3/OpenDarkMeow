...
public final class ColorSpaces {
    ...
    private static final Map<Key, ICC_ColorSpace> cache = new LRUHashMap<Key, ICC_ColorSpace>(16);
    ...
    static ICC_ColorSpace getInternalCS(int n2, byte[] byArray) {
        if (n2 == 5 && Arrays.equals(byArray, ColorProfiles.sRGB.header)) {
            return (ICC_ColorSpace)ColorSpace.getInstance(1000);
        }
        if (n2 == 6 && Arrays.equals(byArray, ColorProfiles.GRAY.header)) {
            return (ICC_ColorSpace)ColorSpace.getInstance(1003);
        }
        if (n2 == 13 && Arrays.equals(byArray, ColorProfiles.PYCC.header)) {
            return (ICC_ColorSpace)ColorSpace.getInstance(1002);
        }
        if (n2 == 5 && Arrays.equals(byArray, ColorProfiles.LINEAR_RGB.header)) {
            return (ICC_ColorSpace)ColorSpace.getInstance(1004);
        }
        if (n2 == 0 && Arrays.equals(byArray, ColorProfiles.CIEXYZ.header)) {
     ...
     * WARNING - Removed try catching itself - possible behaviour change.
     */
    public static ColorSpace getColorSpace(int n2) {
        switch (n2) {
            case 5000: {
        ...
        return ColorSpace.getInstance(n2);
    }
