        ...
        for (UserConnection connection : Via.getManager().getConnectionManager().getConnections()) {
            ...
            for (WorldBorderEmulator.Side side : WorldBorderEmulator.Side.values()) {
                ...
                if (side.modX != 0) {
                    ...
                    d2 = Math.abs(worldBorderEmulatorTracker.getX() + radius * (double)side.modX - playerSession.getPosX());
                } else {
                    center = worldBorderEmulatorTracker.getX();
                    pos = playerSession.getPosX();
                    d2 = Math.abs(worldBorderEmulatorTracker.getZ() + radius * (double)side.modZ - playerSession.getPosZ());
                }
                if (d2 >= 16.0) continue;
                double r2 = Math.sqrt(256.0 - d2 * d2);
                double minH = Math.ceil(pos - r2);
                double maxH = Math.floor(pos + r2);
                double minV = Math.ceil(playerSession.getPosY() - r2);
                double maxV = Math.floor(playerSession.getPosY() + r2);
                ...
                try {
                    ...
                    ViaRewind.getPlatform().getLogger().log(Level.SEVERE, "Failed to send world border particle", e2);
                }
            }
        }
    }
