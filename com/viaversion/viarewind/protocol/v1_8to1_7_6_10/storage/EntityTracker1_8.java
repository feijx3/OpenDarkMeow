...
extends EntityTrackerBase {
    ...
    public void removeEntity(int entityId) {
        ...
        VirtualHologramEntity hologram = (VirtualHologramEntity)this.holograms.get(entityId);
        if (hologram != null) {
            hologram.deleteEntity();
            this.holograms.remove(entityId);
        }
    ...
    public int getVehicle(int passengerId) {
        for (Map.Entry entry : this.vehicles.int2IntEntrySet()) {
            if ((Integer)entry.getValue() != passengerId) continue;
    ...
    protected void startSneaking() {
        try {
            ...
            ViaRewind.getPlatform().getLogger().log(Level.SEVERE, "Failed to send sneak packet", e2);
        }
    }
    ...
    protected void attachEntity(int target) {
        try {
            ...
            ViaRewind.getPlatform().getLogger().log(Level.SEVERE, "Failed to send attach packet", e2);
        }
    }
