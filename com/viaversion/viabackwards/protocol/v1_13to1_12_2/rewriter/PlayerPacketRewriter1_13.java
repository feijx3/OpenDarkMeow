...
extends RewriterBase<Protocol1_13To1_12_2> {
    ...
    protected void registerPackets() {
            ...
            if (channel.equals("minecraft:trader_list")) {
                ...
                if (oldChannel == null) {
                    if (!Via.getConfig().isSuppressConversionWarnings()) {
                        ((Protocol1_13To1_12_2)this.protocol).getLogger().warning(PlayerPacketRewriter1_13.jvmdowngrader$concat$lambda$registerPackets$0$1(channel));
                    }
                    wrapper.cancel();
                    return;
                }
                ...
                if (oldChannel.equals("REGISTER") || oldChannel.equals("UNREGISTER")) {
                    ...
                    for (String s2 : channels) {
                        String rewritten = ItemPacketRewriter1_13.getOldPluginChannelId(s2);
                        if (rewritten != null) {
                            rewrittenChannels.add(rewritten);
                            continue;
                        }
                        if (Via.getConfig().isSuppressConversionWarnings()) continue;
                        ((Protocol1_13To1_12_2)this.protocol).getLogger().warning(PlayerPacketRewriter1_13.jvmdowngrader$concat$lambda$registerPackets$0$1(oldChannel, s2));
                    }
            ...
            public void register() {
                ...
                this.handler(wrapper -> {
                    ...
                    if (data != null) {
                        ...
                        for (int i2 : data) {
                            wrapper.write(Types.VAR_INT, i2);
                        }
                    }
                });
            }
                ...
                this.handler(wrapper -> {
                    ...
                    if (action == 0 || action == 2) {
                        ...
                        if (ViaBackwards.getConfig().addTeamColorTo1_13Prefix()) {
                            prefix = 5.jvmdowngrader$concat$lambda$register$0$1(prefix, colour > -1 && colour <= 15 ? Integer.toHexString(colour) : "r");
                        }
            ...
            for (int i2 = 0; i2 < size; ++i2) {
                ...
                if (nodeType == 1 || nodeType == 2) {
                    ...
                    if (nodeType == 1 && initialNodes) {
                        storage.commands().add(PlayerPacketRewriter1_13.jvmdowngrader$concat$lambda$registerPackets$1$1(name));
                    }
                }
                if (nodeType == 2) {
        ...
        ((Protocol1_13To1_12_2)this.protocol).registerClientbound(ClientboundPackets1_13.COMMAND_SUGGESTIONS, wrapper -> {
            ...
            for (int i2 = 0; i2 < count; ++i2) {
                ...
                wrapper.write(Types.STRING, PlayerPacketRewriter1_13.jvmdowngrader$concat$lambda$registerPackets$2$1(start == 0 && !storage.isLastAssumeCommand() ? "/" : "", match));
        ...
        ((Protocol1_13To1_12_2)this.protocol).registerServerbound(ServerboundPackets1_12_1.CUSTOM_PAYLOAD, wrapper -> {
            ...
            switch (channel = wrapper.read(Types.STRING)) {
                ...
                case "MC|AutoCmd": {
                    String mode;
                    wrapper.setPacketType(ServerboundPackets1_13.SET_COMMAND_BLOCK);
                    int x2 = wrapper.read(Types.INT);
                    int y2 = wrapper.read(Types.INT);
                    int z2 = wrapper.read(Types.INT);
                    wrapper.write(Types.BLOCK_POSITION1_8, new BlockPosition(x2, (short)y2, z2));
                    wrapper.passthrough(Types.STRING);
                    byte flags = 0;
                    if (wrapper.read(Types.BOOLEAN).booleanValue()) {
                        flags = (byte)(flags | 1);
                    }
                    int modeId = (mode = wrapper.read(Types.STRING)).equals("SEQUENCE") ? 0 : (mode.equals("AUTO") ? 1 : 2);
                ...
                case "MC|Struct": {
                    ...
                    int x3 = wrapper.read(Types.INT);
                    int y3 = wrapper.read(Types.INT);
                    int z3 = wrapper.read(Types.INT);
                    wrapper.write(Types.BLOCK_POSITION1_8, new BlockPosition(x3, (short)y3, z3));
                ...
                default: {
                    ...
                    if (newChannel == null) {
                        if (!Via.getConfig().isSuppressConversionWarnings()) {
                            ((Protocol1_13To1_12_2)this.protocol).getLogger().warning(PlayerPacketRewriter1_13.jvmdowngrader$concat$lambda$registerPackets$4$1(channel));
                        }
                        wrapper.cancel();
                        return;
                    }
                    ...
                    ArrayList<String> rewrittenChannels = new ArrayList<String>();
                    for (String s2 : channels) {
                        String rewritten = ItemPacketRewriter1_13.getNewPluginChannelId(s2);
                        if (rewritten != null) {
                            rewrittenChannels.add(rewritten);
                            continue;
                        }
                        if (Via.getConfig().isSuppressConversionWarnings()) continue;
                        ((Protocol1_13To1_12_2)this.protocol).getLogger().warning(PlayerPacketRewriter1_13.jvmdowngrader$concat$lambda$registerPackets$4$1(Key.stripMinecraftNamespace(newChannel).toUpperCase(Locale.ROOT), s2));
                    }
                    if (!rewrittenChannels.isEmpty()) {
                        wrapper.write(Types.SERVERBOUND_CUSTOM_PAYLOAD_DATA, Joiner.on((char)'\u0000').join(rewrittenChannels).getBytes(StandardCharsets.UTF_8));
                        break;
                    }
                    wrapper.cancel();
                }
    ...
    private static String jvmdowngrader$concat$lambda$registerPackets$4$1(String string) {
        return "Ignoring serverbound plugin message with channel: " + string;
    }

    private static String jvmdowngrader$concat$lambda$registerPackets$4$1(String string, String string2) {
        return "Ignoring plugin channel in serverbound " + string + ": " + string2;
    }

    private static String jvmdowngrader$concat$lambda$registerPackets$2$1(String string, String string2) {
        return string + string2;
    }

    private static String jvmdowngrader$concat$lambda$registerPackets$1$1(String string) {
        return "/" + string;
    }

    private static String jvmdowngrader$concat$lambda$registerPackets$0$1(String string) {
        return "Ignoring clientbound plugin message with channel: " + string;
    }

    private static String jvmdowngrader$concat$lambda$registerPackets$0$1(String string, String string2) {
        return "Ignoring plugin channel in clientbound " + string + ": " + string2;
    }
}
