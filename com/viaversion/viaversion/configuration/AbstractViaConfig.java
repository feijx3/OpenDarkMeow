...
implements ViaVersionConfig {
    ...
    protected void loadFields() {
        ...
        this.suppressTextComponentConversionWarnings = this.getBoolean("suppress-text-component-conversion-warnings", true);
    ...
    private BlockedProtocolVersions loadBlockedProtocolVersions() {
        ...
        for (String s2 : blockVersions) {
            ...
            if (c2 == '<' || c2 == '>') {
                ...
                if (c2 == '<') {
                    if (lowerBound.isKnown()) {
                        this.logger.warning(AbstractViaConfig.jvmdowngrader$concat$loadBlockedProtocolVersions$1(String.valueOf(lowerBound), protocolVersion.getName()));
                    }
                    lowerBound = protocolVersion;
                    continue;
                }
                if (upperBound.isKnown()) {
                    this.logger.warning(AbstractViaConfig.jvmdowngrader$concat$loadBlockedProtocolVersions$2(String.valueOf(upperBound), protocolVersion.getName()));
                }
            ...
            this.logger.warning(AbstractViaConfig.jvmdowngrader$concat$loadBlockedProtocolVersions$1(String.valueOf(protocolVersion)));
        }
        if (lowerBound.isKnown() || upperBound.isKnown()) {
            ...
            blockedProtocols.removeIf(version -> {
                if (finalLowerBound.isKnown() && version.olderThan(finalLowerBound) || finalUpperBound.isKnown() && version.newerThan(finalUpperBound)) {
                    this.logger.warning(AbstractViaConfig.jvmdowngrader$concat$lambda$loadBlockedProtocolVersions$0$1(String.valueOf(version)));
                    return true;
                }
                return false;
            });
    ...
    private @Nullable ProtocolVersion protocolVersion(String s2) {
        ...
        if (protocolVersion == null) {
            this.logger.warning(AbstractViaConfig.jvmdowngrader$concat$protocolVersion$1(s2));
            return null;
        }
    ...
    public boolean isSuppressTextComponentConversionWarnings() {
        return this.suppressTextComponentConversionWarnings && !Via.getManager().isDebug();
    }
    ...
    private static String jvmdowngrader$concat$loadBlockedProtocolVersions$1(String string, String string2) {
        return "Already set lower bound " + string + " overridden by " + string2;
    }

    private static String jvmdowngrader$concat$loadBlockedProtocolVersions$2(String string, String string2) {
        return "Already set upper bound " + string + " overridden by " + string2;
    }

    private static String jvmdowngrader$concat$loadBlockedProtocolVersions$1(String string) {
        return "Duplicated blocked protocol version " + string;
    }

    private static String jvmdowngrader$concat$protocolVersion$1(String string) {
        return "Unknown protocol version in block-versions: " + string;
    }

    private static String jvmdowngrader$concat$lambda$loadBlockedProtocolVersions$0$1(String string) {
        return "Blocked protocol version " + string + " already covered by upper or lower bound";
    }
}
