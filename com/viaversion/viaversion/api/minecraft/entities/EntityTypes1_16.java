...
public enum EntityTypes1_16 implements EntityType
{
    ...
    public int getId() {
        if (this.id == -1) {
            throw new IllegalStateException(EntityTypes1_16.jvmdowngrader$concat$getId$1(this.name()));
        }
        return this.id;
    }
    ...
    public static void initialize(Protocol<?, ?, ?, ?> protocol) {
        EntityTypeUtil.initialize((EntityType[])EntityTypes1_16.values(), (EntityType[])TYPES, protocol, (type, id) -> {
            type.id = id;
        });
    }

    static {
        TYPES = EntityTypeUtil.createSizedArray(EntityTypes1_16.values());
    }
