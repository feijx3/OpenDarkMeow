...
public enum EntityTypes1_21_4 implements EntityType
{
    ...
    private EntityTypes1_21_4(EntityType parent) {
        this.parent = parent;
        this.identifier = Key.namespaced(this.name().toLowerCase(Locale.ROOT));
    }

    private EntityTypes1_21_4(EntityType parent, String identifier) {
        this.parent = parent;
    ...
    public int getId() {
        if (this.id == -1) {
            throw new IllegalStateException(EntityTypes1_21_4.jvmdowngrader$concat$getId$1(this.name()));
        }
        return this.id;
    }
    ...
    public static void initialize(Protocol<?, ?, ?, ?> protocol) {
        EntityTypeUtil.initialize((EntityType[])EntityTypes1_21_4.values(), (EntityType[])TYPES, protocol, (type, id) -> {
            type.id = id;
        });
    }

    static {
        TYPES = EntityTypeUtil.createSizedArray(EntityTypes1_21_4.values());
    }
