...
@NestMembers(value={ObjectType.class, EntityType.class})
public class EntityTypes1_12 {

    @NestHost(value=EntityTypes1_12.class)
    public static enum ObjectType implements com.viaversion.viaversion.api.minecraft.entities.ObjectType
    {
        ...
        private static final Map<Integer, Map<Integer, ObjectType>> TYPES;
        ...
        private ObjectType(int id, int data, EntityType type) {
            this.id = id;
            this.data = data;
            this.type = type;
        }
        ...
        public static ObjectType findById(int id, int data) {
            Map<Integer, ObjectType> types = TYPES.get(id);
            if (types == null) {
                return null;
            }
            ObjectType type = types.get(data);
            return type != null ? type : types.get(0);
        }

        public static EntityType getEntityType(int id, int data) {
            ObjectType objectType = ObjectType.findById(id, data);
            return objectType != null ? objectType.type : null;
        }

        static {
            TYPES = new HashMap<Integer, Map<Integer, ObjectType>>();
            for (ObjectType type : ObjectType.values()) {
                TYPES.computeIfAbsent(type.id, k2 -> new HashMap()).put(type.data, type);
            }
        }
    }
