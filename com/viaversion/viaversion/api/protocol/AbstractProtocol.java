...
implements Protocol<CU, CM, SM, SU> {
    ...
    public void registerFinishConfiguration(CU packetType, PacketHandler handler) {
        this.registerClientbound(packetType, wrapper -> {
    ...
    public void registerServerbound(State state, int unmappedPacketId, int mappedPacketId, PacketHandler handler, boolean override) {
        ...
        if (!override && this.serverboundMappings.hasMapping(state, unmappedPacketId)) {
            Via.getPlatform().getLogger().log(Level.WARNING, AbstractProtocol.jvmdowngrader$concat$registerServerbound$1(unmappedPacketId), new Exception());
        }
    ...
    public void registerClientbound(State state, int unmappedPacketId, int mappedPacketId, PacketHandler handler, boolean override) {
        ...
        if (!override && this.clientboundMappings.hasMapping(state, unmappedPacketId)) {
            Via.getPlatform().getLogger().log(Level.WARNING, AbstractProtocol.jvmdowngrader$concat$registerClientbound$1(unmappedPacketId), new Exception());
        }
    ...
    private void register(PacketMappings packetMappings, PacketType packetType, @Nullable PacketType mappedPacketType, Class<? extends PacketType> unmappedPacketClass, Class<? extends PacketType> mappedPacketClass, @Nullable PacketHandler handler, boolean override) {
        ...
        if (!override && packetMappings.hasMapping(packetType)) {
            this.getLogger().log(Level.WARNING, AbstractProtocol.jvmdowngrader$concat$register$1(String.valueOf(packetType)), new Exception());
        }
    ...
    private static <U extends PacketType, M extends PacketType> M mappedPacketType(U packetType, PacketTypeMap<M> mappedTypes, Class<U> unmappedPacketTypeClass, Class<M> mappedPacketTypeClass) {
        ...
        throw new IllegalArgumentException(AbstractProtocol.jvmdowngrader$concat$mappedPacketType$1(String.valueOf(packetType), packetType.getClass().getSimpleName()));
    }
    ...
    public void transform(Direction direction, State state, PacketWrapper packetWrapper) throws InformativeException, CancelException {
        ...
        if (handler != null) {
            try {
                ...
                this.printRemapError(direction, state, unmappedId, packetWrapper.getId(), e2);
                throw e2;
            }
            catch (Exception e3) {
                InformativeException ex2 = new InformativeException(e3);
                ex2.addSource(handler.getClass());
                this.printRemapError(direction, state, unmappedId, packetWrapper.getId(), ex2);
                throw ex2;
            }
    ...
    private void printRemapError(Direction direction, State state, int unmappedPacketId, int mappedPacketId, InformativeException e2) {
        Object packetType;
        if (state != State.PLAY && direction == Direction.SERVERBOUND && !Via.getManager().debugHandler().enabled()) {
            e2.setShouldBePrinted(false);
            return;
        }
        ...
        if (packetType != null) {
            Via.getPlatform().getLogger().warning(AbstractProtocol.jvmdowngrader$concat$printRemapError$1(this.getClass().getSimpleName(), String.valueOf(packetType), ProtocolUtil.toNiceHex(unmappedPacketId)));
        } else {
            Via.getPlatform().getLogger().warning(AbstractProtocol.jvmdowngrader$concat$printRemapError$1(this.getClass().getSimpleName(), String.valueOf((Object)state), ProtocolUtil.toNiceHex(unmappedPacketId), ProtocolUtil.toNiceHex(mappedPacketId)));
        }
    }

    private static void checkPacketType(PacketType packetType, boolean isValid) {
        if (!isValid) {
            throw new IllegalArgumentException(AbstractProtocol.jvmdowngrader$concat$checkPacketType$1(String.valueOf(packetType), packetType.getClass().getSimpleName()));
        }
    }
    ...
    public String toString() {
        return AbstractProtocol.jvmdowngrader$concat$toString$1(this.getClass().getSimpleName());
    }

    private static String jvmdowngrader$concat$registerServerbound$1(int n2) {
        return n2 + " already registered! If this override is intentional, set override to true. Stacktrace: ";
    }

    private static String jvmdowngrader$concat$registerClientbound$1(int n2) {
        return n2 + " already registered! If override is intentional, set override to true. Stacktrace: ";
    }

    private static String jvmdowngrader$concat$register$1(String string) {
        return string + " already registered! If override is intentional, set override to true. Stacktrace: ";
    }

    private static String jvmdowngrader$concat$mappedPacketType$1(String string, String string2) {
        return "Packet type " + string + " in " + string2 + " could not be automatically mapped!";
    }

    private static String jvmdowngrader$concat$printRemapError$1(String string, String string2, String string3) {
        return "ERROR IN " + string + " IN REMAP OF " + string2 + " (" + string3 + ")";
    }

    private static String jvmdowngrader$concat$printRemapError$1(String string, String string2, String string3, String string4) {
        return "ERROR IN " + string + " IN REMAP OF " + string2 + " " + string3 + "->" + string4;
    }

    private static String jvmdowngrader$concat$checkPacketType$1(String string, String string2) {
        return "Packet type " + string + " in " + string2 + " is taken from the wrong packet types class";
    }
