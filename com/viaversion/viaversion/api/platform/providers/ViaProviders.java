...
public class ViaProviders {
    ...
    public <T extends Provider> @Nullable T get(Class<T> provider) {
        ...
        if (this.lonelyProviders.contains(provider)) {
            throw new IllegalStateException(ViaProviders.jvmdowngrader$concat$get$1(String.valueOf(provider)));
        }
        return null;
    }

    private static String jvmdowngrader$concat$get$1(String string) {
        return "There was no provider for " + string + ", one is required!";
    }
}
