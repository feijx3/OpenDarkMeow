...
implements ViaInjector {
    ...
    public void uninject() throws ReflectiveOperationException {
        for (ChannelFuture channelFuture : this.injectedFutures) {
            ...
            for (String name : pipeline.names()) {
                ...
                if (handler == null) {
                    Via.getPlatform().getLogger().warning(LegacyViaInjector.jvmdowngrader$concat$uninject$1(name));
                    continue;
                }
                try {
            ...
            try {
                ...
                Via.getPlatform().getLogger().log(Level.SEVERE, "Failed to remove injection handler, reload won't work with connections, please reboot!", e2);
            }
        }
    ...
    private static String jvmdowngrader$concat$uninject$1(String string) {
        return "Could not get handler " + string;
    }
}
