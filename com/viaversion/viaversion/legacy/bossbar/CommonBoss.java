...
@NestMembers(value={1.class, UpdateAction.class})
...
implements BossBar {
    ...
    public CommonBoss(String title, float health, BossColor color, BossStyle style) {
        ...
        Preconditions.checkArgument((health >= 0.0f && health <= 1.0f ? 1 : 0) != 0, (Object)CommonBoss.jvmdowngrader$concat$$init$$1(health));
    ...
    private void sendPacketConnection(UserConnection conn, PacketWrapper wrapper) {
        ...
        try {
            ...
            Via.getPlatform().getLogger().log(Level.WARNING, "Failed to send bossbar packet", e2);
        }
    }
    ...
    private PacketWrapper getPacket(UpdateAction action, UserConnection connection) {
        try {
            ...
            Via.getPlatform().getLogger().log(Level.WARNING, "Failed to create bossbar packet", e4);
            return null;
        }
    }
    ...
    private static String jvmdowngrader$concat$$init$$1(float f2) {
        return "Health must be between 0 and 1. Input: " + f2;
    }

    @NestHost(value=CommonBoss.class)
