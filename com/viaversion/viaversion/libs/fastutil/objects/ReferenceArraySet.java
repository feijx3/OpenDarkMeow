...
Cloneable {
    ...
    public ReferenceArraySet(Object[] a2) {
        this.a = a2;
        this.size = a2.length;
    }
    ...
    public ReferenceArraySet(ReferenceCollection<K> c2) {
        this(c2.size());
        this.addAll(c2);
    }

    public ReferenceArraySet(Collection<? extends K> c2) {
        this(c2.size());
        this.addAll(c2);
    }

    public ReferenceArraySet(ReferenceSet<K> c2) {
        this(c2.size());
        int i2 = 0;
        for (Object x2 : c2) {
            this.a[i2] = x2;
    ...
    public ReferenceArraySet(Set<? extends K> c2) {
        this(c2.size());
        int i2 = 0;
        for (K x2 : c2) {
            this.a[i2] = x2;
    ...
    public ReferenceArraySet(Object[] a2, int size) {
        this.a = a2;
        this.size = size;
        if (size > a2.length) {
    ...
    public static <K> ReferenceArraySet<K> of(K e2) {
        return ReferenceArraySet.ofUnchecked(e2);
    }

    @SafeVarargs
    public static <K> ReferenceArraySet<K> of(K ... a2) {
        if (a2.length == 2) {
            if (a2[0] == a2[1]) {
                throw new IllegalArgumentException("Duplicate element: " + a2[1]);
            }
        } else if (a2.length > 2) {
            ReferenceOpenHashSet.of(a2);
        }
        return ReferenceArraySet.ofUnchecked(a2);
    }
    ...
    @SafeVarargs
    public static <K> ReferenceArraySet<K> ofUnchecked(K ... a2) {
        return new ReferenceArraySet<K>(a2);
    }
    ...
    public ObjectIterator<K> iterator() {
        return new ObjectIterator<K>(){
            ...
            public int skip(int n2) {
                ...
                n2 = Math.min(n2, ReferenceArraySet.this.size - this.next);
                this.next += n2;
                if (n2 != 0) {
            ...
            public void forEachRemaining(Consumer<? super K> action) {
                Object[] a2 = ReferenceArraySet.this.a;
    ...
    public ReferenceArraySet<K> clone() {
        ReferenceArraySet c2;
        try {
            c2 = (ReferenceArraySet)super.clone();
        }
