...
Hash {
    ...
    public ReferenceLinkedOpenHashSet(int expected, float f2) {
        if (f2 <= 0.0f || f2 >= 1.0f) {
    ...
    public ReferenceLinkedOpenHashSet(Collection<? extends K> c2, float f2) {
        this(c2.size(), f2);
        this.addAll(c2);
    }

    public ReferenceLinkedOpenHashSet(Collection<? extends K> c2) {
        this(c2, 0.75f);
    }

    public ReferenceLinkedOpenHashSet(ReferenceCollection<? extends K> c2, float f2) {
        this(c2.size(), f2);
        this.addAll(c2);
    }

    public ReferenceLinkedOpenHashSet(ReferenceCollection<? extends K> c2) {
        this(c2, 0.75f);
    }

    public ReferenceLinkedOpenHashSet(Iterator<? extends K> i2, float f2) {
        this(16, f2);
        while (i2.hasNext()) {
            this.add(i2.next());
        }
    ...
    public ReferenceLinkedOpenHashSet(Iterator<? extends K> i2) {
        this(i2, 0.75f);
    }

    public ReferenceLinkedOpenHashSet(K[] a2, int offset, int length, float f2) {
    ...
    public ReferenceLinkedOpenHashSet(K[] a2, int offset, int length) {
        this(a2, offset, length, 0.75f);
    }

    public ReferenceLinkedOpenHashSet(K[] a2, float f2) {
        this(a2, 0, a2.length, f2);
    }

    public ReferenceLinkedOpenHashSet(K[] a2) {
        this(a2, 0.75f);
    }
    ...
    public static <K> ReferenceLinkedOpenHashSet<K> of(K e2) {
    ...
    @SafeVarargs
    public static <K> ReferenceLinkedOpenHashSet<K> of(K ... a2) {
        ReferenceLinkedOpenHashSet<K> result = new ReferenceLinkedOpenHashSet<K>(a2.length, 0.75f);
        for (K element : a2) {
    ...
    public ReferenceLinkedOpenHashSet<K> clone() {
        ReferenceLinkedOpenHashSet c2;
        try {
            c2 = (ReferenceLinkedOpenHashSet)super.clone();
        }
    ...
    implements ObjectListIterator<K> {
        ...
        public void remove() {
            ...
            if (this.prev == -1) {
                ...
                int n2 = this.prev;
                ReferenceLinkedOpenHashSet.this.link[n2] = ReferenceLinkedOpenHashSet.this.link[n2] ^ (ReferenceLinkedOpenHashSet.this.link[this.prev] ^ (long)this.next & 0xFFFFFFFFL) & 0xFFFFFFFFL;
            }
            if (this.next == -1) {
                ReferenceLinkedOpenHashSet.this.last = this.prev;
            } else {
                int n3 = this.next;
                ReferenceLinkedOpenHashSet.this.link[n3] = ReferenceLinkedOpenHashSet.this.link[n3] ^ (ReferenceLinkedOpenHashSet.this.link[this.next] ^ ((long)this.prev & 0xFFFFFFFFL) << 32) & 0xFFFFFFFF00000000L;
            }
            int pos = this.curr;
            this.curr = -1;
