...
WritableByteChannel {
    ...
    public int read(byte[] b2, int offset, int length) throws IOException {
        ...
        ByteArrays.ensureOffsetLength((byte[])b2, (int)offset, (int)length);
        int read = 0;
        if (this.position < (long)this.inspectable) {
            int toCopy = Math.min(this.inspectable - (int)this.position, length);
            System.arraycopy(this.buffer, (int)this.position, b2, offset, toCopy);
            length -= toCopy;
            offset += toCopy;
        ...
        if (length > 0) {
            ...
            int t2 = this.randomAccessFile.read(b2, offset, toRead);
            this.position += (long)t2;
            read += t2;
        }
        return read;
    }
    ...
    public long skip(long n2) throws IOException {
        this.ensureOpen();
        long toSkip = Math.min(n2, this.length() - this.position);
