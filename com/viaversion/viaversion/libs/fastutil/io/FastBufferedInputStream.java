...
implements RepositionableStream {
    ...
    public int read(byte[] b2, int offset, int length) throws IOException {
        if (length <= this.avail) {
            System.arraycopy(this.buffer, this.pos, b2, offset, length);
            this.pos += length;
            this.avail -= length;
        ...
        int head = this.avail;
        System.arraycopy(this.buffer, this.pos, b2, offset, head);
        this.avail = 0;
        this.pos = 0;
        this.readBytes += (long)head;
        if (length > this.buffer.length) {
            int result = this.is.read(b2, offset + head, length - head);
            if (result > 0) {
        ...
        System.arraycopy(this.buffer, 0, b2, offset + head, toRead);
        this.pos = toRead;
        this.avail -= toRead;
        return toRead + head;
    }
    ...
    private long skipByReading(long n2) throws IOException {
        long toSkip;
        int len;
        for (toSkip = n2; toSkip > 0L && (len = this.is.read(this.buffer, 0, (int)Math.min((long)this.buffer.length, toSkip))) > 0; toSkip -= (long)len) {
        }
        return n2 - toSkip;
    }
    ...
    public long skip(long n2) throws IOException {
        ...
        long toSkip = n2 - (long)this.avail;
        long result = 0L;
        this.avail = 0;
