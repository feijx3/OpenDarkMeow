...
public class HashCommon {
    ...
    public static int maxFill(int n2, float f2) {
        return Math.min((int)Math.ceil((double)n2 * (double)f2), n2 - 1);
    }

    public static long maxFill(long n2, float f2) {
        return Math.min((long)Math.ceil((double)n2 * (double)f2), n2 - 1L);
    }

    public static int arraySize(int expected, float f2) {
        long s2 = Math.max(2L, HashCommon.nextPowerOfTwo((long)Math.ceil((double)expected / (double)f2)));
        if (s2 > 0x40000000L) {
    ...
    public static long bigArraySize(long expected, float f2) {
        return HashCommon.nextPowerOfTwo((long)Math.ceil((double)expected / (double)f2));
    }
}
