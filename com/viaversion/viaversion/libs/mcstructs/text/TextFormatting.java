...
public class TextFormatting {
    ...
    @Nullable
    public static TextFormatting parse(String s2) {
        if (s2.startsWith("#")) {
            try {
                return new TextFormatting(Integer.parseInt(s2.substring(1), 16));
            }
            catch (NumberFormatException e2) {
                return null;
            }
        }
        return TextFormatting.getByName(s2);
    }
    ...
    public static TextFormatting getClosestFormattingColor(int rgb) {
        ...
        for (TextFormatting color : COLORS.values()) {
            ...
            int distance = (r2 - colorR) * (r2 - colorR) + (g2 - (colorG = color.getRgbValue() >> 8 & 0xFF)) * (g2 - colorG) + (b2 - (colorB = color.getRgbValue() & 0xFF)) * (b2 - colorB);
    ...
    public String serialize() {
        if (Type.RGB.equals((Object)this.type)) {
            ...
            result.append('#');
            String hex = Integer.toHexString(this.rgbValue);
            for (int i2 = 0; i2 < 6 - hex.length(); ++i2) {
                result.append('0');
            }
            result.append(hex);
    ...
    public boolean equals(Object o2) {
        ...
        TextFormatting that = (TextFormatting)o2;
    ...
    public String toString() {
        return ToString.of(this).add("type", this.type).add("name", this.name, s2 -> !this.type.equals((Object)Type.RGB)).add("code", Character.valueOf(this.code), c2 -> !this.type.equals((Object)Type.RGB)).add("rgbValue", this.rgbValue, i2 -> this.type.equals((Object)Type.RGB), i2 -> String.format("#%06X", i2)).toString();
    }
