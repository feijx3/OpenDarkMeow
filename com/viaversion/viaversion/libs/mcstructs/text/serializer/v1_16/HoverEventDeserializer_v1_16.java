...
implements JsonDeserializer<HoverEvent> {
    protected final TextComponentSerializer textComponentSerializer;
    protected final SNbt<?> sNbt;

    public HoverEventDeserializer_v1_16(TextComponentSerializer textComponentSerializer, SNbt<?> sNbt) {
    ...
    protected HoverEvent deserialize(HoverEventAction action, JsonElement contents) {
        switch (action) {
            case SHOW_TEXT: {
            ...
            case SHOW_ITEM: {
                if (contents.isJsonPrimitive()) {
                    return new ItemHoverEvent(Identifier.of(contents.getAsString()), 1, null);
                }
