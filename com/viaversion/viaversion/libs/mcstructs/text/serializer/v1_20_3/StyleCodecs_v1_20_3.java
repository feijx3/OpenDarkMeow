...
import com.viaversion.viaversion.libs.mcstructs.text.serializer.v1_20_3.ExtraCodecs_v1_20_3;
import com.viaversion.viaversion.libs.mcstructs.text.serializer.v1_20_3.TextCodecs_v1_20_3;
import com.viaversion.viaversion.libs.mcstructs.text.serializer.v1_20_3.TextVerifier_v1_20_3;
...
public class StyleCodecs_v1_20_3 {
    ...
    public static class HoverEventCodec {
        ...
        private static <T extends HoverEvent> MapCodec<T> createLegacy(BiFunction<DataConverter<?>, TextComponent, Result<T>> constructor) {
            return TextCodecs_v1_20_3.TEXT.converterFlatMap((dataConverter, t2) -> Result.error("Legacy hover events can't be serialized"), constructor).mapCodec("value").required();
        }
        ...
        public static class Entity {
            public static final MapCodec<EntityHoverEvent> MAP_CODEC = MapCodecMerger.codec(Codec.STRING_IDENTIFIER.converterVerified(VerifyingConverter.verify(TextVerifier_v1_20_3.class, TextVerifier_v1_20_3::verifyRegistryEntity, "Invalid entity")).mapCodec("type").required(), EntityHoverEvent.ModernHolder::getType, ExtraCodecs_v1_20_3.LENIENT_UUID.mapCodec("id").required(), EntityHoverEvent.ModernHolder::getUuid, TextCodecs_v1_20_3.TEXT.mapCodec("name").optional().defaulted(null), EntityHoverEvent.ModernHolder::getName, EntityHoverEvent.ModernHolder::new).mapCodec("contents").required().map(EntityHoverEvent::asModern, EntityHoverEvent::new);
            public static final MapCodec<EntityHoverEvent> LEGACY_MAP_CODEC = HoverEventCodec.access$000((converter, component) -> {
                try {
                    ...
                    TextComponent name = rawName == null ? null : (TextComponent)TextCodecs_v1_20_3.TEXT.deserialize(converter.fork(JsonConverter_v1_20_3.INSTANCE), rawName).getOrThrow(JsonParseException::new);
                    Identifier type = Identifier.of(tag.get("type") instanceof StringTag ? ((StringTag)tag.get("type")).getValue() : "");
                    if (!VerifyingConverter.isValid(converter, type, TextVerifier_v1_20_3.class, TextVerifier_v1_20_3::verifyRegistryEntity)) {
        ...
        public static class Item {
            public static final Codec<Identifier> ITEM_CODEC = Codec.STRING_IDENTIFIER.converterVerified(VerifyingConverter.verify(TextVerifier_v1_20_3.class, TextVerifier_v1_20_3::verifyRegistryItem, "Invalid item"));
            public static final Codec<ItemHoverEvent.ModernHolder> FULL_CODEC = MapCodecMerger.codec(ITEM_CODEC.mapCodec("id").required(), ItemHoverEvent.ModernHolder::getId, Codec.INTEGER.mapCodec("count").optional().defaulted(1), ItemHoverEvent.ModernHolder::getCount, ExtraCodecs_v1_20_3.STRING_COMPOUND.mapCodec("tag").optional().defaulted(null), ItemHoverEvent.ModernHolder::getTag, ItemHoverEvent.ModernHolder::new);
            public static final MapCodec<ItemHoverEvent> MAP_CODEC = Codec.either(ITEM_CODEC, FULL_CODEC).map(Either::right, either -> either.xmap(id -> new ItemHoverEvent.ModernHolder((Identifier)id, 1, null), Function.identity())).mapCodec("contents").required().map(ItemHoverEvent::asModern, ItemHoverEvent::new);
            public static final MapCodec<ItemHoverEvent> LEGACY_MAP_CODEC = HoverEventCodec.access$000((converter, component) -> {
                try {
                    CompoundTag tag = SNbt.V1_14.deserialize(component.asUnformattedString());
                    Identifier id = Identifier.of(tag.get("id") instanceof StringTag ? ((StringTag)tag.get("id")).getValue() : "");
                    if (!VerifyingConverter.isValid(converter, id, TextVerifier_v1_20_3.class, TextVerifier_v1_20_3::verifyRegistryItem)) {
                        return Result.error("Invalid item: " + id);
                    }
                    byte count = tag.get("Count") instanceof ByteTag ? ((ByteTag)tag.get("Count")).asByte() : (byte)0;
                    CompoundTag nbt = tag.get("tag") instanceof CompoundTag ? (CompoundTag)tag.get("tag") : null;
