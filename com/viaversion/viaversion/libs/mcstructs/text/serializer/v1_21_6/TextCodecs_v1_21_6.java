...
public class TextCodecs_v1_21_6 {
    public static final Codec<TextComponent> TEXT = new LazyInitCodec<TextComponent>(() -> Codec.recursive(thiz -> TextCodecs_v1_20_3.createCodec((Codec)thiz, StyleCodecs_v1_21_6.MAP_CODEC, (TextCodecs_v1_20_3.TextComponentType[])ComponentType.values(), ComponentType::forComponent)));
    ...
    }), TEXT).map(o2 -> o2 instanceof TextComponent ? Either.right((TextComponent)o2) : Either.left(o2), Either::unwrap).listOf().mapCodec("with").optional().defaulted(List::isEmpty, ArrayList::new), comp -> Arrays.asList(comp.getArgs()), (key, fallback, arguments) -> new TranslationComponent((String)key, (List<?>)arguments).setFallback((String)fallback));
    ...
    public static final MapCodec<SelectorComponent> SELECTOR_COMPONENT = MapCodecMerger.mapCodec(Codec.STRING.converterVerified(VerifyingConverter.verify(TextVerifier_v1_21_6.class, TextVerifier_v1_21_6::verifySelector, "Invalid selector")).mapCodec("selector").required(), SelectorComponent::getSelector, TEXT.mapCodec("separator").optional().defaulted(null), SelectorComponent::getSeparator, SelectorComponent::new);
