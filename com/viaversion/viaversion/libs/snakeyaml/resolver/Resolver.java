...
import com.viaversion.viaversion.libs.snakeyaml.resolver.ResolverTuple;
...
public class Resolver {
    ...
    public Tag resolve(NodeId kind2, String value, boolean implicit) {
        if (kind2 == NodeId.scalar && implicit) {
            Pattern regexp;
            Tag tag;
            ...
            if (resolvers != null) {
                for (ResolverTuple v2 : resolvers) {
                    tag = v2.getTag();
                    regexp = v2.getRegexp();
                    if (value.length() > v2.getLimit() || !regexp.matcher(value).matches()) continue;
                    return tag;
                }
            }
