...
public class Composer {
    ...
    public Node getNode() {
        ...
        if (this.parser.checkEvent(Event.ID.StreamEnd)) {
            ...
            List<NodeTuple> children2 = Collections.emptyList();
            MappingNode node = new MappingNode(Tag.COMMENT, false, children2, startMark, null, DumperOptions.FlowStyle.BLOCK);
    ...
    protected Node composeScalarNode(String anchor, List<CommentLine> blockComments) {
        ...
        if (tag == null || tag.equals("!")) {
            ...
            if (nodeTag.isCustomGlobal() && !this.loadingConfig.getTagInspector().isGlobalTagAllowed(nodeTag)) {
                throw new ComposerException(null, null, "Global tag is not allowed: " + tag, ev2.getStartMark());
            }
        }
    ...
    protected Node composeMappingNode(String anchor) {
        ...
        while (!this.parser.checkEvent(Event.ID.MappingEnd)) {
            ...
            this.composeMappingChildren(children2, node);
        }
        if (startEvent.isFlow()) {
            node.setInLineComments(this.inlineCommentsCollector.collectEvents().consume());
        }
    ...
    protected void composeMappingChildren(List<NodeTuple> children2, MappingNode node) {
