...
implements TypeAdapterFactory {
    ...
    private boolean includeField(Field f2, boolean serialize) {
        return !this.excluder.excludeField(f2, serialize);
    }
    ...
    private List<String> getFieldNames(Field f2) {
        ...
        if (annotation == null) {
            fieldName = this.fieldNamingPolicy.translateName(f2);
            alternates = this.fieldNamingPolicy.alternateNames(f2);
        } else {
    ...
    private FieldsData getBoundFields(Gson context, TypeToken<?> type, Class<?> raw, boolean blockInaccessible, boolean isRecord) {
        ...
        while (raw != Object.class) {
            Field[] fields2 = raw.getDeclaredFields();
            if (raw != originalRaw && fields2.length > 0) {
                ReflectionAccessFilter.FilterResult filterResult = ReflectionAccessFilterHelper.getFilterResult(this.reflectionFilters, raw);
                if (filterResult == ReflectionAccessFilter.FilterResult.BLOCK_ALL) {
