...
implements ViaPlatformLoader {
    ...
    public void load() {
        ...
        if (serverProtocolVersion.olderThan(ProtocolVersion.v1_14)) {
            boolean use1_9Fix;
            boolean bl2 = use1_9Fix = plugin.getConf().is1_9HitboxFix() && serverProtocolVersion.olderThan(ProtocolVersion.v1_9);
            if (use1_9Fix || plugin.getConf().is1_14HitboxFix()) {
                try {
                    ...
                    Via.getPlatform().getLogger().log(Level.WARNING, "Could not load hitbox fix - please report this on our GitHub", e2);
                }
            }
        }
        ...
        if (serverProtocolVersion.olderThan(ProtocolVersion.v1_9)) {
            ...
            Via.getManager().getProviders().use(HandItemProvider.class, new HandItemProvider(){
                ...
                public Item getHandItem(UserConnection info) {
                    if (BukkitViaLoader.this.jvmdowngrader$nest$com_viaversion_viaversion_bukkit_platform_BukkitViaLoader$get$handItemCache() != null) {
                        return BukkitViaLoader.this.jvmdowngrader$nest$com_viaversion_viaversion_bukkit_platform_BukkitViaLoader$get$handItemCache().getHandItem(info.getProtocolInfo().getUuid());
                    }
                    try {
                        ...
                        Via.getPlatform().getLogger().log(Level.SEVERE, "Error fetching hand item", e2);
                        return null;
                    }
                }
            });
    ...
    public HandItemCache jvmdowngrader$nest$com_viaversion_viaversion_bukkit_platform_BukkitViaLoader$get$handItemCache() {
        return this.handItemCache;
    }

    public void jvmdowngrader$nest$com_viaversion_viaversion_bukkit_platform_BukkitViaLoader$set$handItemCache(HandItemCache handItemCache) {
        this.handItemCache = handItemCache;
    }
}
