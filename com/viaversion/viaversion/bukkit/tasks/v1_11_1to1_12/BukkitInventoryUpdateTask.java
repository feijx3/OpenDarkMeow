...
implements Runnable {
    ...
    @Override
    public void run() {
        Player p2 = Bukkit.getServer().getPlayer(this.uuid);
        if (p2 == null) {
        ...
        try {
            ...
            synchronized (list) {
                ...
                while (iterator2.hasNext() && (result = this.provider.sendPacketToServer(p2, packet = this.provider.buildWindowClickPacket(p2, storage = iterator2.next())))) {
                }
                this.items.clear();
            }
        }
