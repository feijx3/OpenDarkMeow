...
extends InventoryQuickMoveProvider {
    ...
    public boolean registerQuickMoveAction(short windowId, short slotId, short actionId, UserConnection userConnection) {
        ...
        boolean bl2 = registered = updateTask != null;
        if (!registered) {
    ...
    public Object buildWindowClickPacket(Player p2, ItemTransactionStorage storage) {
        ProtocolVersion protocol;
        ...
        if (slotId <= inv.countSlots()) {
            ...
            String cause = BukkitInventoryQuickMoveProvider.jvmdowngrader$concat$buildWindowClickPacket$1(slotId, inv.countSlots(), String.valueOf(inv.getType()), String.valueOf(tinvtype));
            Via.getPlatform().getLogger().severe(BukkitInventoryQuickMoveProvider.jvmdowngrader$concat$buildWindowClickPacket$1(cause));
        }
        Object packet = null;
        try {
            ...
            Via.getPlatform().getLogger().log(Level.SEVERE, BukkitInventoryQuickMoveProvider.jvmdowngrader$concat$buildWindowClickPacket$2(e2.getMessage()), e2);
        }
        return packet;
    }
    ...
    public boolean sendPacketToServer(Player p2, Object packet) {
        ...
        try {
            ...
            Via.getPlatform().getLogger().log(Level.SEVERE, "Failed to send packet to server", e2);
            return false;
        }
        return true;
    }
    ...
    private static String jvmdowngrader$concat$buildWindowClickPacket$1(short s2, int n2, String string, String string2) {
        return "Too many inventory slots: slotId: " + s2 + " invSlotCount: " + n2 + " invType: " + string + " topInvType: " + string2;
    }

    private static String jvmdowngrader$concat$buildWindowClickPacket$1(String string) {
        return "Failed to get an item to create a window click packet. Please report this issue to the ViaVersion Github: " + string;
    }

    private static String jvmdowngrader$concat$buildWindowClickPacket$2(String string) {
        return "Failed to create a window click packet. Please report this issue to the ViaVersion Github: " + string;
    }
}
