...
@Plugin(id="viaversion", name="ViaVersion", version="5.4.2", authors={"_MylesC", "creeper123123321", "Gerrygames", "kennytv", "Matsv", "EnZaXD", "RK_01"}, description="Allows the connection of newer clients to older server versions for Minecraft servers.", url="https://viaversion.com")
...
implements ViaServerProxyPlatform<Player> {
    ...
    public void sendCustomPayload(UserConnection connection, String channel, byte[] message) {
        UUID uuid = connection.getProtocolInfo().getUuid();
        PROXY.getPlayer(uuid).flatMap(Player::getCurrentServer).ifPresent(server -> server.sendPluginMessage((ChannelIdentifier)MinecraftChannelIdentifier.from((String)channel), message));
    }
    ...
    public JsonObject getDump() {
        ...
        for (PluginContainer p2 : PROXY.getPluginManager().getPlugins()) {
            plugins.add(new PluginInfo(true, p2.getDescription().getName().orElse(p2.getDescription().getId()), p2.getDescription().getVersion().orElse("Unknown Version"), p2.getInstance().map(instance -> instance.getClass().getCanonicalName()).orElse("Unknown"), p2.getDescription().getAuthors()));
        }
