...
implements ViaVersionCommand {
    ...
    public void registerSubCommand(ViaSubCommand command) {
        Preconditions.checkArgument((boolean)command.name().matches("^[a-z0-9_-]{3,15}$"), (Object)ViaCommandHandler.jvmdowngrader$concat$registerSubCommand$1(command.name()));
        Preconditions.checkArgument((!this.hasSubCommand(command.name()) ? 1 : 0) != 0, (Object)ViaCommandHandler.jvmdowngrader$concat$registerSubCommand$2(command.name()));
    ...
    public boolean onCommand(ViaCommandSender sender, String[] args) {
        ...
        if (!result) {
            sender.sendMessage(ViaCommandHandler.jvmdowngrader$concat$onCommand$1(handler.usage()));
        }
        return result;
    }
        ...
        if (args.length == 1) {
            if (!args[0].isEmpty()) {
                for (ViaSubCommand sub : allowed) {
                    if (!sub.name().toLowerCase(Locale.ROOT).startsWith(args[0].toLowerCase(Locale.ROOT))) continue;
                    output.add(sub.name());
                }
            } else {
                for (ViaSubCommand sub : allowed) {
            ...
            if (!tab.isEmpty()) {
                ...
                for (String s2 : tab) {
                    if (!s2.toLowerCase(Locale.ROOT).startsWith(currArg.toLowerCase(Locale.ROOT))) continue;
                    output.add(s2);
                }
            }
            return output;
    ...
    public void showHelp(ViaCommandSender sender) {
        ...
        sender.sendMessage(ViaSubCommand.color(ViaCommandHandler.jvmdowngrader$concat$showHelp$1(Via.getPlatform().getPluginVersion())));
    ...
    private static String jvmdowngrader$concat$registerSubCommand$1(String string) {
        return string + " is not a valid sub-command name.";
    }

    private static String jvmdowngrader$concat$registerSubCommand$2(String string) {
        return "ViaSubCommand " + string + " does already exists!";
    }

    private static String jvmdowngrader$concat$onCommand$1(String string) {
        return "Usage: /viaversion " + string;
    }

    private static String jvmdowngrader$concat$showHelp$1(String string) {
        return "&aViaVersion &c" + string;
    }
}
