...
implements ViaSubCommand {
    ...
    public boolean execute(ViaCommandSender sender, String[] args) {
        ...
        if (args.length == 1) {
            ...
            if (args[0].equalsIgnoreCase("logpretransform")) {
                debug.setLogPrePacketTransform(!debug.logPrePacketTransform());
                this.sendMessage(sender, "&6Pre transform packet logging is now %s", debug.logPrePacketTransform() ? "&aenabled" : "&cdisabled");
                return true;
            }
            ...
            if (args[0].equalsIgnoreCase("add")) {
                ...
                this.sendMessage(sender, "&6Added packet type %s to debug logging", args[1]);
                return true;
            }
            if (args[0].equalsIgnoreCase("remove")) {
                debug.removePacketTypeNameToLog(args[1].toUpperCase(Locale.ROOT));
                this.sendMessage(sender, "&6Removed packet type %s from debug logging", args[1]);
                return true;
            }
        }
        return false;
