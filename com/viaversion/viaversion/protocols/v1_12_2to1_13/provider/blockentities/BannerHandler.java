...
implements BlockEntityProvider.BlockEntityHandler {
    ...
    public int transform(UserConnection user, CompoundTag tag) {
        ...
        if (!storage.contains(position = new BlockPosition(tag.getNumberTag("x").asInt(), tag.getNumberTag("y").asShort(), tag.getNumberTag("z").asInt()))) {
            Protocol1_12_2To1_13.LOGGER.warning(BannerHandler.jvmdowngrader$concat$transform$1(String.valueOf(tag)));
            return -1;
        }
        int blockId = storage.get(position).getOriginal();
        NumberTag base = tag.getNumberTag("Base");
        int n2 = color = base != null ? base.asInt() : 0;
        if (blockId >= 6854 && blockId <= 7109) {
            blockId += (15 - color) * 16;
        } else if (blockId >= 7110 && blockId <= 7173) {
            blockId += (15 - color) * 4;
        } else {
            Protocol1_12_2To1_13.LOGGER.warning(BannerHandler.jvmdowngrader$concat$transform$2(String.valueOf(tag)));
        }
    ...
    private static String jvmdowngrader$concat$transform$1(String string) {
        return "Received an banner color update packet, but there is no banner! O_o " + string;
    }

    private static String jvmdowngrader$concat$transform$2(String string) {
        return "Why does this block have the banner block entity? :(" + string;
    }
}
