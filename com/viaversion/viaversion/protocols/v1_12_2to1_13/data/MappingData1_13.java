...
extends MappingDataBase {
        ...
        if ((object = MappingDataLoader.INSTANCE.loadFromDataDir("channelmappings-1.13.json")) != null) {
            for (Map.Entry<String, JsonElement> entry : object.entrySet()) {
                ...
                if (!Key.isValid(newChannel)) {
                    this.getLogger().warning(MappingData1_13.jvmdowngrader$concat$loadExtras$1(newChannel));
                    continue;
                }
    ...
    protected @Nullable BiMappings loadBiMappings(CompoundTag data, String key) {
        if (key.equals("items")) {
            return (BiMappings)MappingDataLoader.INSTANCE.loadMappings(data, "items", size -> {
                ...
                return map;
            }, Int2IntBiHashMap::put, (v2, mappedSize) -> Int2IntMapBiMappings.of(v2));
        }
    ...
    private static String jvmdowngrader$concat$loadExtras$1(String string) {
        return "Channel '" + string + "' is not a valid 1.13 plugin channel, please check your configuration!";
    }
}
