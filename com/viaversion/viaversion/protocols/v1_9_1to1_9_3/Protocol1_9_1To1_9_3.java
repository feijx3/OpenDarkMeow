    ...
    protected void registerPackets() {
        this.registerClientbound(ClientboundPackets1_9.UPDATE_SIGN, null, (PacketWrapper wrapper) -> {
            ...
            for (int i2 = 0; i2 < 4; ++i2) {
                lines[i2] = wrapper.read(Types.COMPONENT);
            }
            ...
            for (int i3 = 0; i3 < lines.length; ++i3) {
                tag.put(Protocol1_9_1To1_9_3.jvmdowngrader$concat$lambda$registerPackets$0$1(i3 + 1), new StringTag(lines[i3].toString()));
            }
        ...
        this.registerClientbound(ClientboundPackets1_9.LEVEL_CHUNK, wrapper -> {
            ...
            for (int s2 = 0; s2 < chunk.getSections().length; ++s2) {
                ...
                for (int idx = 0; idx < 4096; ++idx) {
                    ...
                    tags.add(FakeTileEntities1_9_1.createTileEntity(ChunkSection.xFromIndex(idx) + (chunk.getX() << 4), ChunkSection.yFromIndex(idx) + (s2 << 4), ChunkSection.zFromIndex(idx) + (chunk.getZ() << 4), id));
                }
            }
        });
