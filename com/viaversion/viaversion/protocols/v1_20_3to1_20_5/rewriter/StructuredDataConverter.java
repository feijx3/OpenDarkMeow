...
@NestMembers(value={DataConverter.class, SimpleDataConverter.class})
public final class StructuredDataConverter {
    ...
    public StructuredDataConverter(boolean backupInconvertibleData) {
        ...
        this.register(StructuredDataKey.ATTRIBUTE_MODIFIERS1_20_5, (T data, CompoundTag tag) -> {
            ...
            for (int i2 = 0; i2 < data.modifiers().length; ++i2) {
                AttributeModifiers1_20_5.AttributeModifier modifier = data.modifiers()[i2];
        ...
        this.register(StructuredDataKey.CUSTOM_MODEL_DATA1_20_5, (T data, CompoundTag tag) -> tag.putInt("CustomModelData", (int)data));
        ...
        this.register(StructuredDataKey.WRITABLE_BOOK_CONTENT, (T data, CompoundTag tag) -> {
            ...
            for (int i2 = 0; i2 < data.pages().length; ++i2) {
                FilterableString page = data.pages()[i2];
                pages.add(new StringTag((String)page.raw()));
                if (page.filtered() == null) continue;
                filteredPages.putString(Integer.toString(i2), (String)page.filtered());
            }
            tag.put("pages", pages);
        ...
        this.register(StructuredDataKey.WRITTEN_BOOK_CONTENT, (T data, CompoundTag tag) -> {
            ...
            CompoundTag filteredPages = new CompoundTag();
            for (int i2 = 0; i2 < data.pages().length; ++i2) {
                FilterableComponent page = data.pages()[i2];
                pages.add(new StringTag(ComponentUtil.tagToJsonString((Tag)page.raw())));
                if (page.filtered() == null) continue;
                filteredPages.putString(Integer.toString(i2), ComponentUtil.tagToJsonString((Tag)page.filtered()));
            }
            tag.put("pages", pages);
            tag.put("filtered_pages", filteredPages);
        ...
        this.register(StructuredDataKey.PROFILE, (T data, CompoundTag tag) -> {
            ...
            for (GameProfile.Property property : data.properties()) {
                ListTag<CompoundTag> values = new ListTag<CompoundTag>(CompoundTag.class);
        ...
        this.register(StructuredDataKey.TRIM1_20_5, (UserConnection connection, T data, CompoundTag tag) -> {
            ...
            if (data.material().isDirect()) {
                ...
                if (!material.overrideArmorMaterials().isEmpty()) {
                    for (Map.Entry<String, String> entry : material.overrideArmorMaterials().entrySet()) {
    ...
    private void convertBlockPredicates(CompoundTag tag, AdventureModePredicate data, String key, int hideFlag) {
        ...
        for (BlockPredicate predicate : data.predicates()) {
            ...
            if (holders.hasTagKey()) {
                String tagKey = StructuredDataConverter.jvmdowngrader$concat$convertBlockPredicates$1(holders.tagKey());
    ...
    private void convertItemList(UserConnection connection, Item[] items, CompoundTag tag, String key) {
        ...
        for (int i2 = 0; i2 < items.length; ++i2) {
            ...
            if (this.backupInconvertibleData) {
                savedItem.putByte("Slot", (byte)i2);
            }
    ...
    private <T> void register(StructuredDataKey<T> key, SimpleDataConverter<T> converter) {
        DataConverter<Object> c2 = (connection, data, tag) -> converter.convert(data, tag);
        this.rewriters.put(key, c2);
    }
    ...
    private static String jvmdowngrader$concat$convertBlockPredicates$1(String string) {
        return "#" + string;
    }

    @FunctionalInterface
    @NestHost(value=StructuredDataConverter.class)
