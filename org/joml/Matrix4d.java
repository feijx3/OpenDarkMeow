...
Matrix4dc {
    ...
    private Matrix4d rotationInternal(double angle, double x2, double y2, double z2) {
        ...
        this._m00((double)(cos + x2 * x2 * C))._m10((double)(xy * C - z2 * sin))._m20((double)(xz * C + y2 * sin))._m01((double)(xy * C + z2 * sin))._m11((double)(cos + y2 * y2 * C))._m21((double)(yz * C - x2 * sin))._m02((double)(xz * C - y2 * sin))._m12((double)(yz * C + x2 * sin))._m22((double)(cos + z2 * z2 * C)).properties = 18;
        return this;
    }
    ...
    public Vector3f transformDirection(double x2, double y2, double z2, Vector3f dest) {
        float rx = (float)(this.m00 * x2 + this.m10 * y2 + this.m20 * z2);
        float ry = (float)(this.m01 * x2 + this.m11 * y2 + this.m21 * z2);
        float rz = (float)(this.m02 * x2 + this.m12 * y2 + this.m22 * z2);
        dest.x = rx;
        dest.y = ry;
        dest.z = rz;
        return dest;
    ...
    public Vector4d transformAffine(double x2, double y2, double z2, double w2, Vector4d dest) {
        double rx = this.m00 * x2 + this.m10 * y2 + this.m20 * z2 + this.m30 * w2;
        double ry = this.m01 * x2 + this.m11 * y2 + this.m21 * z2 + this.m31 * w2;
        double rz = this.m02 * x2 + this.m12 * y2 + this.m22 * z2 + this.m32 * w2;
        dest.x = rx;
        dest.y = ry;
        dest.z = rz;
        dest.w = w2;
    ...
    private Matrix4d rotateGenericInternal(double ang, double x2, double y2, double z2, Matrix4d dest) {
        ...
        double zz = z2 * z2;
        double rm00 = xx * C + c2;
        double rm01 = xy * C + z2 * s2;
        double rm02 = xz * C - y2 * s2;
        double rm10 = xy * C - z2 * s2;
        double rm11 = yy * C + c2;
        double rm12 = yz * C + x2 * s2;
        double rm20 = xz * C + y2 * s2;
        double rm21 = yz * C - x2 * s2;
        double rm22 = zz * C + c2;
    ...
    public Matrix4d rotateTranslation(double ang, double x2, double y2, double z2, Matrix4d dest) {
        ...
        double tz = this.m32;
        if (y2 == 0.0 && z2 == 0.0 && Math.absEqualsOne(x2)) {
            return dest.rotationX(x2 * ang).setTranslation(tx, ty, tz);
        }
        if (x2 == 0.0 && z2 == 0.0 && Math.absEqualsOne(y2)) {
            return dest.rotationY(y2 * ang).setTranslation(tx, ty, tz);
        }
        if (x2 == 0.0 && y2 == 0.0 && Math.absEqualsOne(z2)) {
            return dest.rotationZ(z2 * ang).setTranslation(tx, ty, tz);
        }
    ...
    private Matrix4d rotateLocalGenericInternal(double ang, double x2, double y2, double z2, Matrix4d dest) {
        ...
        double zz = z2 * z2;
        double lm00 = xx * C + c2;
        double lm01 = xy * C + z2 * s2;
        double lm02 = xz * C - y2 * s2;
        double lm10 = xy * C - z2 * s2;
        double lm11 = yy * C + c2;
        double lm12 = yz * C + x2 * s2;
        double lm20 = xz * C + y2 * s2;
        double lm21 = yz * C - x2 * s2;
        double lm22 = zz * C + c2;
    ...
    public Matrix4d translationRotateScale(double tx, double ty, double tz, double qx, double qy, double qz, double qw2, double sx, double sy, double sz) {
        double dqx = qx + qx;
        double dqy = qy + qy;
        double dqz = qz + qz;
        double q00 = dqx * qx;
    ...
    public Matrix4d translationRotateScale(double tx, double ty, double tz, double qx, double qy, double qz, double qw2, double scale) {
        return this.translationRotateScale(tx, ty, tz, qx, qy, qz, qw2, scale, scale, scale);
    }
    ...
    public Matrix4d translationRotateScaleInvert(double tx, double ty, double tz, double qx, double qy, double qz, double qw2, double sx, double sy, double sz) {
        boolean one;
        boolean bl2 = one = Math.absEqualsOne(sx) && Math.absEqualsOne(sy) && Math.absEqualsOne(sz);
        if (one) {
            return this.translationRotateInvert(tx, ty, tz, qx, qy, qz, qw2);
        }
        double nqx = -qx;
        double nqy = -qy;
        double nqz = -qz;
    ...
    public Matrix4d translationRotateScaleMulAffine(double tx, double ty, double tz, double qx, double qy, double qz, double qw2, double sx, double sy, double sz, Matrix4d m2) {
        double w2 = qw2 * qw2;
        double x2 = qx * qx;
        double y2 = qy * qy;
        double z2 = qz * qz;
        ...
        this.m21 = m21;
        this.m23 = 0.0;
        double m30 = nm00 * m2.m30 + nm10 * m2.m31 + nm20 * m2.m32 + tx;
        double m31 = nm01 * m2.m30 + nm11 * m2.m31 + nm21 * m2.m32 + ty;
        this.m32 = nm02 * m2.m30 + nm12 * m2.m31 + nm22 * m2.m32 + tz;
        this.m30 = m30;
        this.m31 = m31;
        this.m33 = 1.0;
        boolean one = Math.absEqualsOne(sx) && Math.absEqualsOne(sy) && Math.absEqualsOne(sz);
    ...
    public Matrix4d translationRotate(double tx, double ty, double tz, double qx, double qy, double qz, double qw2) {
        double w2 = qw2 * qw2;
        double x2 = qx * qx;
        double y2 = qy * qy;
        double z2 = qz * qz;
    ...
    public Matrix4d translationRotateInvert(double tx, double ty, double tz, double qx, double qy, double qz, double qw2) {
        double nqx = -qx;
        double nqy = -qy;
        double nqz = -qz;
        double dqx = nqx + nqx;
    ...
    public Vector4d project(double x2, double y2, double z2, int[] viewport, Vector4d winCoordsDest) {
        ...
        double nx = Math.fma(this.m00, x2, Math.fma(this.m10, y2, Math.fma(this.m20, z2, this.m30))) * invW;
        double ny = Math.fma(this.m01, x2, Math.fma(this.m11, y2, Math.fma(this.m21, z2, this.m31))) * invW;
        double nz = Math.fma(this.m02, x2, Math.fma(this.m12, y2, Math.fma(this.m22, z2, this.m32))) * invW;
    ...
    public Matrix4d tile(int x2, int y2, int w2, int h2, Matrix4d dest) {
        float tx = w2 - 1 - (x2 << 1);
        float ty = h2 - 1 - (y2 << 1);
        return dest._m30(Math.fma(this.m00, (double)tx, Math.fma(this.m10, (double)ty, this.m30)))._m31(Math.fma(this.m01, (double)tx, Math.fma(this.m11, (double)ty, this.m31)))._m32(Math.fma(this.m02, (double)tx, Math.fma(this.m12, (double)ty, this.m32)))._m33(Math.fma(this.m03, (double)tx, Math.fma(this.m13, (double)ty, this.m33)))._m00(this.m00 * (double)w2)._m01(this.m01 * (double)w2)._m02(this.m02 * (double)w2)._m03(this.m03 * (double)w2)._m10(this.m10 * (double)h2)._m11(this.m11 * (double)h2)._m12(this.m12 * (double)h2)._m13(this.m13 * (double)h2)._m20(this.m20)._m21(this.m21)._m22(this.m22)._m23(this.m23)._properties(this.properties & 0xFFFFFFE2);
    }
    ...
    public Matrix4d pick(double x2, double y2, double width, double height, int[] viewport, Matrix4d dest) {
        ...
        double tx = ((double)viewport[2] + 2.0 * ((double)viewport[0] - x2)) / width;
        double ty = ((double)viewport[3] + 2.0 * ((double)viewport[1] - y2)) / height;
    ...
    public Matrix4d frustumAabb(Vector3d min, Vector3d max) {
        ...
        for (int t2 = 0; t2 < 8; ++t2) {
            ...
            double invW = 1.0 / (this.m03 * x2 + this.m13 * y2 + this.m23 * z2 + this.m33);
            double nx = (this.m00 * x2 + this.m10 * y2 + this.m20 * z2 + this.m30) * invW;
            double ny = (this.m01 * x2 + this.m11 * y2 + this.m21 * z2 + this.m31) * invW;
            double nz = (this.m02 * x2 + this.m12 * y2 + this.m22 * z2 + this.m32) * invW;
    ...
    public Matrix4d orthoCrop(Matrix4dc view, Matrix4d dest) {
        ...
        for (int t2 = 0; t2 < 8; ++t2) {
            ...
            double z2 = (double)((t2 >>> 2 & 1) << 1) - 1.0;
            double invW = 1.0 / (this.m03 * x2 + this.m13 * y2 + this.m23 * z2 + this.m33);
            double wx = (this.m00 * x2 + this.m10 * y2 + this.m20 * z2 + this.m30) * invW;
            double wy = (this.m01 * x2 + this.m11 * y2 + this.m21 * z2 + this.m31) * invW;
            double wz = (this.m02 * x2 + this.m12 * y2 + this.m22 * z2 + this.m32) * invW;
    ...
    public Matrix4d trapezoidCrop(double p0x, double p0y, double p1x, double p1y, double p2x, double p2y, double p3x, double p3y) {
        ...
        nm31 += d2;
        double sx = 2.0 / d2x;
        double sy = 1.0 / (c3y + d2);
        double u2 = (sy + sy) * d2 / (1.0 - sy * d2);
        double m03 = nm01 * sy;
        double m13 = nm11 * sy;
        double m33 = nm31 * sy;
        nm01 = (u2 + 1.0) * m03;
    ...
    public static void perspectiveOffCenterViewFromRectangle(Vector3d eye, Vector3d p2, Vector3d x2, Vector3d y2, double nearFarDist, boolean zeroToOne, Matrix4d projDest, Matrix4d viewDest) {
        double far;
        double zx = y2.y * x2.z - y2.z * x2.y;
        double zy = y2.z * x2.x - y2.x * x2.z;
        double zz = y2.x * x2.y - y2.y * x2.x;
        double zd = zx * (p2.x - eye.x) + zy * (p2.y - eye.y) + zz * (p2.z - eye.z);
        double zs = zd >= 0.0 ? 1.0 : -1.0;
        zd *= zs;
        viewDest.setLookAt(eye.x, eye.y, eye.z, eye.x + (zx *= zs), eye.y + (zy *= zs), eye.z + (zz *= zs), y2.x, y2.y, y2.z);
        double px = viewDest.m00 * p2.x + viewDest.m10 * p2.y + viewDest.m20 * p2.z + viewDest.m30;
        double py = viewDest.m01 * p2.x + viewDest.m11 * p2.y + viewDest.m21 * p2.z + viewDest.m31;
        double tx = viewDest.m00 * x2.x + viewDest.m10 * x2.y + viewDest.m20 * x2.z;
        double ty = viewDest.m01 * y2.x + viewDest.m11 * y2.y + viewDest.m21 * y2.z;
