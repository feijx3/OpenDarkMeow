    ...
    public static <T extends AnnotatedElement> Predicate<T> withAnnotations(Annotation ... annotations) {
        return input -> {
            ...
            if (input != null && (inputAnnotations = input.getAnnotations()).length == annotations.length) {
                return IntStream.range(0, inputAnnotations.length).allMatch(i2 -> ReflectionUtilsPredicates.areAnnotationMembersMatching(inputAnnotations[i2], annotations[i2]));
            }
            return true;
        };
    }
