...
implements IClassGenerator {
    ...
    private byte[] generateClass(String name, String desc) {
        ...
        ClassVisitor visitor2 = writer;
        if (MixinEnvironment.getCurrentEnvironment().getOption(MixinEnvironment.Option.DEBUG_VERIFY)) {
            visitor2 = new CheckClassAdapter(writer);
        }
        visitor2.visit(50, 4129, ref, null, ARGS_REF, null);
        visitor2.visitSource(name.substring(name.lastIndexOf(46) + 1) + ".java", null);
        this.generateCtor(ref, desc, args, visitor2);
        this.generateToString(ref, desc, args, visitor2);
        this.generateFactory(ref, desc, args, visitor2);
        this.generateSetters(ref, desc, args, visitor2);
        this.generateGetters(ref, desc, args, visitor2);
        visitor2.visitEnd();
    ...
    private void generateToString(String ref, String desc, Type[] args, ClassVisitor writer) {
        ...
        toString2.visitCode();
        toString2.visitLdcInsn("Args" + ArgsClassGenerator.getSignature(args));
