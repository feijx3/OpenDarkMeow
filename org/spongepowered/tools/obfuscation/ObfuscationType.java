...
public final class ObfuscationType {
    ...
    private ObfuscationType(ObfuscationTypeDescriptor descriptor2, IMixinAnnotationProcessor ap2) {
        this.key = descriptor2.getKey();
    ...
    public static ObfuscationType create(ObfuscationTypeDescriptor descriptor2, IMixinAnnotationProcessor ap2) {
        String key = descriptor2.getKey();
        if (types.containsKey(key)) {
            throw new IllegalArgumentException("Obfuscation type with key " + key + " was already registered");
        }
        ObfuscationType type = new ObfuscationType(descriptor2, ap2);
        types.put(key, type);
        return type;
    }
