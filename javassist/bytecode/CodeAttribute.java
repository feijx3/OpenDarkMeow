...
implements Opcode {
    ...
    CodeAttribute(ConstPool cp2, int name_id, DataInputStream in) throws IOException {
        ...
        this.exceptions = new ExceptionTable(cp2, in);
        this.attributes = new ArrayList<AttributeInfo>();
        int num = in.readUnsignedShort();
        for (int i2 = 0; i2 < num; ++i2) {
            this.attributes.add(AttributeInfo.read(cp2, in));
        }
    }
    ...
    public void insertLocalVar(int where, int size) throws BadBytecode {
        CodeIterator ci2 = this.iterator();
        while (ci2.hasNext()) {
            CodeAttribute.shiftIndex(ci2, where, size);
        }
        this.setMaxLocals(this.getMaxLocals() + size);
    }

    private static void shiftIndex(CodeIterator ci2, int lessThan, int delta) throws BadBytecode {
        int index = ci2.next();
        int opcode = ci2.byteAt(index);
        if (opcode < 21) {
            return;
        }
        if (opcode < 79) {
            if (opcode < 26) {
                CodeAttribute.shiftIndex8(ci2, index, opcode, lessThan, delta);
            } else if (opcode < 46) {
                CodeAttribute.shiftIndex0(ci2, index, opcode, lessThan, delta, 26, 21);
            } else {
                if (opcode < 54) {
                    return;
                }
                if (opcode < 59) {
                    ...
                    CodeAttribute.shiftIndex0(ci2, index, opcode, lessThan, delta, 59, 54);
                }
            }
        } else if (opcode == 132) {
            int var = ci2.byteAt(index + 1);
            if (var < lessThan) {
                return;
            }
            if ((var += delta) < 256) {
                ci2.writeByte(var, index + 1);
            } else {
                byte plus = (byte)ci2.byteAt(index + 2);
                int pos = ci2.insertExGap(3);
                ci2.writeByte(196, pos - 3);
                ci2.writeByte(132, pos - 2);
                ci2.write16bit(var, pos - 1);
                ci2.write16bit(plus, pos + 1);
            }
        } else if (opcode == 169) {
            CodeAttribute.shiftIndex8(ci2, index, opcode, lessThan, delta);
        } else if (opcode == 196) {
            int var = ci2.u16bitAt(index + 2);
            if (var < lessThan) {
                return;
            }
            ci2.write16bit(var += delta, index + 2);
        }
    }

    private static void shiftIndex8(CodeIterator ci2, int index, int opcode, int lessThan, int delta) throws BadBytecode {
        int var = ci2.byteAt(index + 1);
        if (var < lessThan) {
            return;
        }
        if ((var += delta) < 256) {
            ...
            int pos = ci2.insertExGap(2);
            ci2.writeByte(196, pos - 2);
            ci2.writeByte(opcode, pos - 1);
            ci2.write16bit(var, pos);
        }
    }

    private static void shiftIndex0(CodeIterator ci2, int index, int opcode, int lessThan, int delta, int opcode_i_0, int opcode_i) throws BadBytecode {
        int var = (opcode - opcode_i_0) % 4;
        if (var < lessThan) {
            return;
        }
        if ((var += delta) < 4) {
            ci2.writeByte(opcode + delta, index);
        } else {
            opcode = (opcode - opcode_i_0) / 4 + opcode_i;
            if (var < 256) {
                int pos = ci2.insertExGap(1);
                ci2.writeByte(opcode, pos - 1);
                ci2.writeByte(var, pos);
            } else {
                int pos = ci2.insertExGap(3);
                ci2.writeByte(196, pos - 1);
                ci2.writeByte(opcode, pos);
                ci2.write16bit(var, pos + 1);
            }
        }
    }
