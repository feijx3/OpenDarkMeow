...
public final class ClassFile {
    ...
    private void testExistingField(String name, String descriptor2) throws DuplicateMemberException {
    ...
    private void testExistingMethod(MethodInfo newMinfo) throws DuplicateMemberException {
        ...
        String descriptor2 = newMinfo.getDescriptor();
        ListIterator<MethodInfo> it = this.methods.listIterator(0);
        while (it.hasNext()) {
            if (!ClassFile.isDuplicated(newMinfo, name, descriptor2, it.next(), it)) continue;
    ...
    private void read(DataInputStream in) throws IOException {
        ...
        this.fields = new ArrayList<FieldInfo>();
        for (i2 = 0; i2 < n2; ++i2) {
            this.addField2(new FieldInfo(cp2, in));
        }
        n2 = in.readUnsignedShort();
        this.methods = new ArrayList<MethodInfo>();
        for (i2 = 0; i2 < n2; ++i2) {
            this.addMethod2(new MethodInfo(cp2, in));
        }
        this.attributes = new ArrayList<AttributeInfo>();
        n2 = in.readUnsignedShort();
        for (i2 = 0; i2 < n2; ++i2) {
            this.addAttribute(AttributeInfo.read(cp2, in));
        }
