...
extends AttributeInfo {
    ...
    public TypeAnnotationsAttribute(ConstPool cp2, String attrname, byte[] info) {
        super(cp2, attrname, info);
    }

    TypeAnnotationsAttribute(ConstPool cp2, int n2, DataInputStream in) throws IOException {
        super(cp2, n2, in);
    }
    ...
    extends AnnotationsAttribute.Renamer {
        ...
        Renamer(byte[] attrInfo, ConstPool cp2, Map<String, String> map) {
            super(attrInfo, cp2, map);
    ...
    static class SubWalker {
        ...
        int typePath(int pos, int pathLength) throws Exception {
            for (int i2 = 0; i2 < pathLength; ++i2) {
                int kind2 = this.info[pos] & 0xFF;
                int index = this.info[pos + 1] & 0xFF;
                this.typePath(pos, kind2, index);
                pos += 2;
            }
            return pos;
        }
