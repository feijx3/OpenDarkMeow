...
implements Opcode {
    ...
    public int computeMaxStack() throws BadBytecode {
        boolean repeat;
        CodeIterator ci2 = this.codeAttr.iterator();
        int length = ci2.getCodeLength();
        ...
        do {
            ...
            for (int i2 = 0; i2 < length; ++i2) {
                ...
                repeat = true;
                this.visitBytecode(ci2, stack, i2);
            }
        } while (repeat);
        int maxStack = 1;
    ...
    private void visitBytecode(CodeIterator ci2, int[] stack, int index) throws BadBytecode {
        ...
        while (ci2.hasNext()) {
            ...
            stackDepth = this.visitInst(op, ci2, index, stackDepth);
            if (stackDepth < 1) {
                throw new BadBytecode("stack underflow at " + index);
            }
            if (this.processBranch(op, ci2, index, codeLength, stack, stackDepth, jsrDepth) || CodeAnalyzer.isEnd(op)) break;
            if (op != 168 && op != 201) continue;
            --stackDepth;
        }
    }

    private boolean processBranch(int opcode, CodeIterator ci2, int index, int codeLength, int[] stack, int stackDepth, int[] jsrDepth) throws BadBytecode {
        if (153 <= opcode && opcode <= 166 || opcode == 198 || opcode == 199) {
            int target = index + ci2.s16bitAt(index + 1);
            ...
            switch (opcode) {
                ...
                case 200: {
                    int target = index + ci2.s32bitAt(index + 1);
                    this.checkTarget(index, target, codeLength, stack, stackDepth);
                    return true;
                }
                case 168: 
                case 201: {
                    int target = opcode == 168 ? index + ci2.s16bitAt(index + 1) : index + ci2.s32bitAt(index + 1);
                    this.checkTarget(index, target, codeLength, stack, stackDepth);
                    if (jsrDepth[0] < 0) {
                ...
                case 171: {
                    ...
                    int target = index + ci2.s32bitAt(index2);
                    this.checkTarget(index, target, codeLength, stack, stackDepth);
                    if (opcode == 171) {
                        int npairs = ci2.s32bitAt(index2 + 4);
                        index2 += 12;
                        for (int i2 = 0; i2 < npairs; ++i2) {
                            target = index + ci2.s32bitAt(index2);
                            this.checkTarget(index, target, codeLength, stack, stackDepth);
                            index2 += 8;
                        }
                    } else {
                        int low = ci2.s32bitAt(index2 + 4);
                        int high = ci2.s32bitAt(index2 + 8);
                        int n2 = high - low + 1;
                        index2 += 12;
    ...
    private int visitInst(int op, CodeIterator ci2, int index, int stack) throws BadBytecode {
        switch (op) {
            case 180: {
                stack += this.getFieldSize(ci2, index) - 1;
                break;
            }
            case 181: {
                stack -= this.getFieldSize(ci2, index) + 1;
            ...
            case 178: {
                stack += this.getFieldSize(ci2, index);
                break;
            }
            case 179: {
                stack -= this.getFieldSize(ci2, index);
            ...
            case 182: 
            case 183: {
                String desc = this.constPool.getMethodrefType(ci2.u16bitAt(index + 1));
            ...
            case 185: {
                String desc = this.constPool.getInterfaceMethodrefType(ci2.u16bitAt(index + 1));
                stack += Descriptor.dataSize(desc) - 1;
                break;
            }
            case 186: {
                String desc = this.constPool.getInvokeDynamicType(ci2.u16bitAt(index + 1));
                stack += Descriptor.dataSize(desc);
                break;
            }
            case 191: {
            ...
            case 197: {
                stack += 1 - ci2.byteAt(index + 3);
                break;
            }
            case 196: {
                op = ci2.byteAt(index + 1);
            }
            default: {
                stack += STACK_GROW[op];
            }
    ...
    private int getFieldSize(CodeIterator ci2, int index) {
        String desc = this.constPool.getFieldrefType(ci2.u16bitAt(index + 1));
