...
implements TypeTag {
    ...
    private void pushMemberType(String descriptor2) {
        int top = 0;
        if (descriptor2.charAt(0) == '(' && (top = descriptor2.indexOf(41) + 1) < 1) {
            throw new IndexOutOfBoundsException("bad descriptor: " + descriptor2);
        }
        TypeData[] types = this.stackTypes;
        int index = this.stackTop;
        switch (descriptor2.charAt(top)) {
            case '[': {
                types[index] = new TypeData.ClassName(descriptor2.substring(top));
                break;
            }
            case 'L': {
                types[index] = new TypeData.ClassName(Tracer.getFieldClassName(descriptor2, top));
                break;
            }
            case 'J': {
                types[index] = LONG;
