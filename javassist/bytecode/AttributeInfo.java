...
public class AttributeInfo {
    ...
    protected AttributeInfo(ConstPool cp2, int n2, DataInputStream in) throws IOException {
        this.constPool = cp2;
        this.name = n2;
        int len = in.readInt();
    ...
    static AttributeInfo read(ConstPool cp2, DataInputStream in) throws IOException {
        int name = in.readUnsignedShort();
        String nameStr = cp2.getUtf8Info(name);
        char first = nameStr.charAt(0);
        if (first < 'E') {
            if (nameStr.equals("AnnotationDefault")) {
                return new AnnotationDefaultAttribute(cp2, name, in);
            }
            if (nameStr.equals("BootstrapMethods")) {
                return new BootstrapMethodsAttribute(cp2, name, in);
            }
            if (nameStr.equals("Code")) {
                return new CodeAttribute(cp2, name, in);
            }
            if (nameStr.equals("ConstantValue")) {
                return new ConstantAttribute(cp2, name, in);
            }
            if (nameStr.equals("Deprecated")) {
                return new DeprecatedAttribute(cp2, name, in);
            }
        }
        if (first < 'M') {
            if (nameStr.equals("EnclosingMethod")) {
                return new EnclosingMethodAttribute(cp2, name, in);
            }
            if (nameStr.equals("Exceptions")) {
                return new ExceptionsAttribute(cp2, name, in);
            }
            if (nameStr.equals("InnerClasses")) {
                return new InnerClassesAttribute(cp2, name, in);
            }
            if (nameStr.equals("LineNumberTable")) {
                return new LineNumberAttribute(cp2, name, in);
            }
            if (nameStr.equals("LocalVariableTable")) {
                return new LocalVariableAttribute(cp2, name, in);
            }
            if (nameStr.equals("LocalVariableTypeTable")) {
                return new LocalVariableTypeAttribute(cp2, name, in);
            }
        }
        if (first < 'S') {
            if (nameStr.equals("MethodParameters")) {
                return new MethodParametersAttribute(cp2, name, in);
            }
            if (nameStr.equals("NestHost")) {
                return new NestHostAttribute(cp2, name, in);
            }
            if (nameStr.equals("NestMembers")) {
                return new NestMembersAttribute(cp2, name, in);
            }
            if (nameStr.equals("RuntimeVisibleAnnotations") || nameStr.equals("RuntimeInvisibleAnnotations")) {
                return new AnnotationsAttribute(cp2, name, in);
            }
            if (nameStr.equals("RuntimeVisibleParameterAnnotations") || nameStr.equals("RuntimeInvisibleParameterAnnotations")) {
                return new ParameterAnnotationsAttribute(cp2, name, in);
            }
            if (nameStr.equals("RuntimeVisibleTypeAnnotations") || nameStr.equals("RuntimeInvisibleTypeAnnotations")) {
                return new TypeAnnotationsAttribute(cp2, name, in);
            }
        }
        if (first >= 'S') {
            if (nameStr.equals("Signature")) {
                return new SignatureAttribute(cp2, name, in);
            }
            if (nameStr.equals("SourceFile")) {
                return new SourceFileAttribute(cp2, name, in);
            }
            if (nameStr.equals("Synthetic")) {
                return new SyntheticAttribute(cp2, name, in);
            }
            if (nameStr.equals("StackMap")) {
                return new StackMap(cp2, name, in);
            }
            if (nameStr.equals("StackMapTable")) {
                return new StackMapTable(cp2, name, in);
            }
        }
        return new AttributeInfo(cp2, name, in);
    }
