...
extends CtClass {
    ...
    public ClassFile getClassFile3(boolean doCompress) {
        ...
        if (rcfile != null) {
            ClassFile cf2;
            try {
                cf2 = new ClassFile(new DataInputStream(new ByteArrayInputStream(rcfile)));
            }
    ...
    public void setGenericSignature(String sig) {
        ...
        SignatureAttribute sa = new SignatureAttribute(cf2.getConstPool(), sig);
        cf2.addAttribute(sa);
    }
    ...
    private void nameReplaced() {
        CtMember.Cache cache2 = this.hasMemberCache();
        if (cache2 != null) {
            CtMember tail = cache2.lastMethod();
            for (CtMember mth = cache2.methodHead(); mth != tail; mth = mth.next()) {
                mth.nameReplaced();
            }
        }
    }
    ...
    protected synchronized CtMember.Cache getMembers() {
        CtMember.Cache cache2 = null;
        if (this.memberCache == null || (cache2 = this.memberCache.get()) == null) {
            cache2 = new CtMember.Cache(this);
            this.makeFieldCache(cache2);
            this.makeBehaviorCache(cache2);
            this.memberCache = new WeakReference<CtMember.Cache>(cache2);
        }
        return cache2;
    }

    private void makeFieldCache(CtMember.Cache cache2) {
        List<FieldInfo> fields2 = this.getClassFile3(false).getFields();
        for (FieldInfo finfo : fields2) {
            cache2.addField(new CtField(finfo, (CtClass)this));
        }
    }

    private void makeBehaviorCache(CtMember.Cache cache2) {
        List<MethodInfo> methods2 = this.getClassFile3(false).getMethods();
        for (MethodInfo minfo : methods2) {
            if (minfo.isMethod()) {
                cache2.addMethod(new CtMethod(minfo, this));
                continue;
            }
            cache2.addConstructor(new CtConstructor(minfo, (CtClass)this));
        }
    }
    ...
    public CtMethod[] getDeclaredMethods() {
        CtMember.Cache memCache = this.getMembers();
        CtMember mthTail = memCache.lastMethod();
        ArrayList<CtMember> methods2 = new ArrayList<CtMember>();
        for (CtMember mth = memCache.methodHead(); mth != mthTail; mth = mth.next()) {
            methods2.add(mth);
        }
        return methods2.toArray(new CtMethod[methods2.size()]);
    }
    ...
    public void instrument(CodeConverter converter) throws CannotCompileException {
        ...
        List<MethodInfo> methods2 = cf2.getMethods();
        for (MethodInfo minfo : methods2.toArray(new MethodInfo[methods2.size()])) {
    ...
    private void modifyClassConstructor(ClassFile cf2) throws CannotCompileException, NotFoundException {
        ...
        while (fi2 != null) {
            ...
            if (Modifier.isStatic(f2.getModifiers())) {
                doInit = true;
                int s2 = fi2.init.compileIfStatic(f2.getType(), f2.getName(), code, jv);
                if (stacksize < s2) {
                    stacksize = s2;
                }
            }
    ...
    private void modifyClassConstructor(ClassFile cf2, Bytecode code, int stacksize, int localsize) throws CannotCompileException {
        ...
        if (m2 == null) {
            ...
            CtMember.Cache cache2 = this.hasMemberCache();
            if (cache2 != null) {
                cache2.addConstructor(new CtConstructor(m2, (CtClass)this));
            }
        } else {
    ...
    private int makeFieldInitializer(Bytecode code, CtClass[] parameters) throws CannotCompileException, NotFoundException {
        ...
        while (fi2 != null) {
            int s2;
            CtField f2 = fi2.field;
            if (!Modifier.isStatic(f2.getModifiers()) && stacksize < (s2 = fi2.init.compile(f2.getType(), f2.getName(), code, parameters, jv))) {
                stacksize = s2;
            }
            fi2 = fi2.next;
        }
    ...
    public String makeUniqueName(String prefix) {
        ...
        String[] methods2 = new String[keys.size()];
        keys.toArray(methods2);
        ...
        } while (!CtClassType.notFindInArray(name = prefix + i2++, methods2));
        return name;
    }
    ...
    private void makeMemberList(Map<Object, CtClassType> table) {
        ...
        List<MethodInfo> methods2 = this.getClassFile2().getMethods();
        for (MethodInfo minfo : methods2) {
            table.put(minfo.getName(), this);
        }
        List<FieldInfo> fields2 = this.getClassFile2().getFields();
        for (FieldInfo finfo : fields2) {
